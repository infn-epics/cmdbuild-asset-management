Ext.define('CMDBuildUI.view.administration.content.tasks.card.helpers.EmailServiceMixin', {
    ...
    emailservice: {
        ...
        getSettingsPanel: function (theVmObject, step, data, ctx) {
            var items = [
                ctx.getRowFieldContainer([ctx.getFilterTypeInput(theVmObject, 'config.filter_type')]),

                ctx.getRowFieldContainer([ctx.getFunctionsInput(theVmObject, 'config.filter_function_name')], {
                    bind: {
                ...
                ctx.getRowFieldContainer([
                    ctx.getActionNotificationAutogenerateActiveInput(
                        theVmObject,
                        'config.action_notification_autogenerated_active'
                    ),
                    ctx.getActionNotificationAutogenerateTemplateInput(
                        theVmObject,
                        'config.action_notification_autogenerated_template'
                    )
                ])
            ];
        ...
        getProcessPanel: function (theVmObject, step, data, ctx) {
            var items = [
                ctx.getRowFieldContainer([ctx.getStartProcessInput(theVmObject, 'config.action_workflow_active')]),
                ...
                ctx.getRowFieldContainer(
                    [
                        ...
                        ctx.getRowFieldContainer(
                            [
                                ctx.getWorkflowSaveAttachmentsInput(
                                    theVmObject,
                                    'config.action_workflow_attachmentssave'
                                ),
                                ctx.getWorkflowAttachmentCategoryInput(
                                    theVmObject,
                                    'config.action_workflow_attachmentscategory'
                                )
                            ],
                            {
                                hidden: true,
                        ...
                        ctx.getRowFieldContainer([ctx.getProcessAttributesGrid(), ctx.getProcessAttributesGridForm()], {
                            bind: {
        ...
        getNotificationPanel: function (theVmObject, step, data, ctx) {
            var items = [
                ctx.getRowFieldContainer([
                    ...
                    ctx.getRowFieldContainer(
                        [
                            ctx.getActionNotificationAttachmentsActiveInput(
                                theVmObject,
                                'config.action_notification_attachments_active'
                            ),
                            ctx.getActionNotificationAttachmentsModeInput(
                                theVmObject,
                                'config.action_notification_attachments_mode',
                                {
                                    fieldcontainer: {
                                        bind: {
                                            hidden: Ext.String.format(
                                                '{!{0}.config.action_notification_attachments_active}',
                                                theVmObject
                                            )
                                        }
                                    }
            ...
            _form.getFields().items.forEach(function (field) {
                ...
                switch (fieldName) {
                    ...
                    case 'action_notification_attachments_mode':
                        if (
                            form.down('[name="action_notification_active"]').getValue() !== false &&
                            form.down('[name="action_notification_attachments_active"]').getValue() !== false
                        ) {
                            CMDBuildUI.util.administration.helper.FieldsHelper.setAllowBlank(field, false, _form);
                        } else {
                            CMDBuildUI.util.administration.helper.FieldsHelper.setAllowBlank(field, true, _form);
