Ext.define('CMDBuildUI.view.administration.content.dashboards.card.CardController', {
    ...
    onBeforeRender: function (view) {
        ...
        vm.bind(
            ...
            function (data) {
                ...
                view.add(
                    CMDBuildUI.view.administration.content.dashboards.card.FormHelper.getGeneralProperties(
                        'display',
                        view
                    )
                );
                var layoutGrid = CMDBuildUI.view.administration.content.dashboards.card.FormHelper.getLayout(
                    'display',
                    view
                );
                view.add(layoutGrid);

                view.down('#layoutcontainer').add([
                    CMDBuildUI.util.administration.helper.GridHelper.getDragAndDropReorderGrid([
                        {
                            xtype: 'widgetcolumn',
                            flex: 1,
                            widget: {
        ...
        theDashboard.save({
            success: function (record, operation) {
                ...
                if (grid) {
                    ...
                    var nextUrl = CMDBuildUI.util.administration.helper.ApiHelper.client.getDashboardUrl(
                        record.getId()
                    );
    ...
    onTranslateClickDescription: function (event, button, eOpts) {
        ...
        var translationCode =
            CMDBuildUI.util.administration.helper.LocalizationHelper.getLocaleKeyOfDashboardDescription(
                vm.get('actions.edit') ? theDashboard.get('name') : '.'
            );
    ...
    onTranslateClickExtDescription: function (event, button, eOpts) {
        ...
        var theDashboard = vm.get('theDashboard');
        var translationCode =
            CMDBuildUI.util.administration.helper.LocalizationHelper.getLocaleKeyOfDashboardExtDescription(
                vm.get('actions.edit') ? theDashboard.get('name') : '.'
            );
        CMDBuildUI.util.administration.helper.FormHelper.openLocalizationPopup(
            translationCode,
    ...
    saveDashboardDescriptionLocales: function (vm, record) {
        if (vm.get('actions.add')) {
            var translations = ['theDescriptionTranslation'];
            var keyFunction = ['getLocaleKeyOfDashboardDescription'];
            Ext.Array.forEach(translations, function (item, index) {
                if (vm.get(item)) {
                    var translationCode = CMDBuildUI.util.administration.helper.LocalizationHelper[keyFunction[index]](
                        record.get('name')
                    );
    ...
    saveDashboardChartLocalizedFieldLocales: function (vm, record) {
        record.charts().each(function (chart) {
            ...
            chart.dataSourceParameters().each(function (parameter, index) {
                const parameterDescriptionLocaleVmObject = Ext.String.format(
                    'theChartParameterDescription_{0}_{1}',
                    idChartReplaced,
                    index
                );
                if (vm.get(parameterDescriptionLocaleVmObject)) {
                    const parameterDescriptionTranslationCode =
                        CMDBuildUI.util.administration.helper.LocalizationHelper.getLocaleKeyOfDashboardChartParameterName(
                            recordName,
                            idChart,
                            index
                        );
                    vm.set(
                        Ext.String.format('{0}._id', parameterDescriptionLocaleVmObject),
                        parameterDescriptionTranslationCode
                    );
            ...
            if (vm.get(chartDescritpionVmObject)) {
                const chartDescritpionTranslationCode =
                    CMDBuildUI.util.administration.helper.LocalizationHelper.getLocaleKeyOfDashboardChartDescription(
                        recordName,
                        idChart
                    );
            ...
            if (vm.get(categoryAxisTitleVmObject)) {
                const categoryAxisTitleTranslationCode =
                    CMDBuildUI.util.administration.helper.LocalizationHelper.getLocaleKeyOfDashboardChartCategoryAxis(
                        recordName,
                        idChart
                    );
            ...
            if (vm.get(valueAxisLabelVmObject)) {
                const valueAxisLabelTranslationCode =
                    CMDBuildUI.util.administration.helper.LocalizationHelper.getLocaleKeyOfDashboardChartValueAxis(
                        recordName,
                        idChart
                    );
            ...
            if (vm.get(labelFieldVmObject)) {
                const labelFieldTranslationCode =
                    CMDBuildUI.util.administration.helper.LocalizationHelper.getLocaleKeyOfDashboardChartLabelField(
                        recordName,
                        idChart
                    );
    ...
    deleteRow: function (view, rowIndex, colIndex, item, e, record, row) {
        ...
        const chartsStore = this.getViewModel().get('theDashboard').charts();

        record.get('columns').forEach(function (column) {
            (column.charts || []).forEach(function (chartId) {
                const chartRecord = chartsStore.findRecord('_id', chartId);
                if (chartRecord) {
                    chartsStore.remove(chartRecord);
                }
            });
        });
            ...
            if (columns.length > 1) {
                ...
                if (!wasRemoved) {
                    CMDBuildUI.util.Notifier.showWarningMessage(
                        CMDBuildUI.locales.Locales.administration.dashboards.unabletoremovenonemptycolumn
                    );
                    return;
                }
                record.set('width', undefined);
