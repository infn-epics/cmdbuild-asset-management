Ext.define('CMDBuildUI.view.relations.list.Grid', {
    ...
    requires: ['CMDBuildUI.view.relations.list.GridController', 'CMDBuildUI.view.relations.list.GridModel'],
    ...
    columns: [
                        ...
                        attributes.getRange().forEach(function (a) {
                            ...
                            if (value) {
                                ...
                                if (
                                    ...
                                    value = CMDBuildUI.util.helper.FieldsHelper.renderAttributeValue(
                                        a.getData(),
                                        value
                                    );
                                }
                                if (
                                    a.get('type').toLowerCase() ===
                                    CMDBuildUI.util.helper.ModelHelper.cmdbuildtypes.boolean.toLowerCase()
                                ) {
                                    value = value
                                        ? CMDBuildUI.locales.Locales.common.attributes.booltrue
                                        : CMDBuildUI.locales.Locales.common.attributes.boolfalse;
                                }
                                items.push(
                                    Ext.String.format(
                                        '{0}: {1}',
        ...
        },
        {
            ...
            items: [
                {
                    iconCls:
                        'relations-grid-action ' +
                        CMDBuildUI.util.helper.IconHelper.getIconId('external-link-alt', 'solid'),
                    handler: function (grid, rowIndex, colIndex, item, eOpts, record) {
                        grid.fireEvent('actionopencard', grid, record, rowIndex, colIndex);
                    },
                    isActionDisabled: function (view, rowIndex, colIndex, item, record) {
                        const titem = CMDBuildUI.util.helper.ModelHelper.getObjectFromName(
                            record.get('_destinationType')
                        );
                        return !titem;
                    },
                ...
                {
                    iconCls:
                        'relations-grid-action ' + CMDBuildUI.util.helper.IconHelper.getIconId('pencil-alt', 'solid'),
                    handler: function (grid, rowIndex, colIndex, item, eOpts, record) {
                    ...
                    isActionDisabled: function (view, rowIndex, colIndex, item, record) {
                        ...
                        return (
                            !record.get('_can_update') ||
                            record.get('_destinationIsProcess') ||
                            vm.get('referenceDomains').includes(recordType) ||
                            !vm.get('editableDomains')[recordType]
                        );
                    },
                    getTip: function (v, metadata, record, rowindex, colindex, store) {
                ...
                {
                    iconCls:
                        'relations-grid-action ' + CMDBuildUI.util.helper.IconHelper.getIconId('trash-alt', 'solid'),
                    handler: function (grid, rowIndex, colIndex, item, eOpts, record) {
            ...
            features: [
                {
                    ...
                    startCollapsed: CMDBuildUI.util.helper.Configurations.get(
