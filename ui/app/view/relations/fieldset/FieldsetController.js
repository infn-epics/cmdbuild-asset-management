Ext.define('CMDBuildUI.view.relations.fieldset.FieldsetController', {
        ...
        vm.bind('{targetmodel}', function (targetmodel) {
            if (targetmodel) {
                ...
                if (vm.get('targettype') === CMDBuildUI.util.helper.ModelHelper.objecttypes.process) {
                    ...
                    vm.set(
                        'relstore.proxyurl',
                        CMDBuildUI.util.api.Processes.getProcessInstanceRelations(
                            vm.get('current.objectTypeName'),
                            vm.get('current.objectId')
                        )
                    );
                } else {
                    ...
                    vm.set(
                        'relstore.proxyurl',
                        CMDBuildUI.util.api.Classes.getCardRelations(
                            vm.get('current.objectTypeName'),
                            vm.get('current.objectId')
                        )
                    );
                                ...
                                vm.get('domain')
                                    ...
                                    .then(function (attrs) {
                                        ...
                                        attrs.getRange().forEach(function (attr) {
                                            const field =
                                                CMDBuildUI.util.helper.ModelHelper.getModelFieldFromAttribute(attr);
                                            const col = CMDBuildUI.util.helper.GridHelper.getColumn(field);
                                            if (col) {
                                                const baserenderer = col.renderer;
                                                col.hidden = false;
                                                col.renderer = function (
                                                    ...
                                                    // find relation item
                                                    const rels = view.lookupViewModel().get('allRelations');
                                                    if (rels) {
                                                        const relPos = rels.findBy(function (relItem) {
                                                            return (
                                                                relItem.get('_type') === vm.get('domain').getId() &&
                                                                relItem.get('_destinationId') == record.get('_id')
                                                            );
                                                        });
                                                        if (relPos !== -1) {
                                                            const relItem = rels.getAt(relPos);
                                                            // return base render
                                                            if (baserenderer) {
                                                                return baserenderer(
                                                                    relItem.get(attr.get('name')),
                                                                    metaData,
                                                                    relItem,
                                                                    rowindex,
                                                                    colindex,
                                            ...
                                            view.add({
                                                ...
                                                maxHeight:
                                                    (CMDBuildUI.util.helper.Configurations.get(
                                                        CMDBuildUI.model.Configuration.ui.relationlimit
                                                    ) +
                                                        1) *
                                                        31 +
                                                    15,
                                                        ...
                                                        widget: {
                                                            ...
                                                            viewModel: {
                                                                data: {
                                                                    ...
                                                                    objectId: null,
                                                                    objectTypeName: null,
                                                                    objectModel: targetmodel,
                                                                    basepermissions: view
                                                                        .lookupViewModel()
                                                                        .get('basepermissions')
                                                                }
                                                            ...
                                                            tabpaneltools: [
                                                                {
                                                                    ...
                                                                    iconCls:
                                                                        CMDBuildUI.util.helper.IconHelper.getIconId(
                                                                            'external-link-alt',
                                                                            'solid'
                                                                        ),
                                                                    cls: 'management-tool',
                                                                    tooltip:
                                                                        CMDBuildUI.locales.Locales.relations.opencard,
                                                                    callback: function (panel, tool, event) {
                                                                        me.onItemViewCardActionClick(panel.up());
                                                                    },
                                                                    autoEl: {
                                                                        'data-testid':
                                                                            'relations-fieldset-viewcardaction'
                                                                    }
                                                                },
                                                                {
                                                                    xtype: 'tool',
                                                                    ...
                                                                    hidden: btnHidden,
                                                                    tooltip:
                                                                        CMDBuildUI.locales.Locales.relations
                                                                            .editrelation,
                                                                    callback: function (panel, tool, event) {
                                                                    ...
                                                                    autoEl: {
                                                                        'data-testid':
                                                                            'relations-fieldset-editrelationcard'
                                                                    },
                                                                    bind: {
                                                                        disabled: '{!sourceOrTargetEditable}'
                                                                    }
                                                                },
                                                                {
                                                                    xtype: 'tool',
                                                                    ...
                                                                    autoEl: {
                                                                        'data-testid':
                                                                            'relations-fieldset-deleterelationcard'
                                                                    },
                                                                    bind: {
                                                                        disabled: '{!sourceOrTargetEditable}'
                                                                    }
                                                                },
                                                                {
                                                                    ...
                                                                    hidden: btnHidden,
                                                                    tooltip:
                                                                        CMDBuildUI.locales.Locales.relations.editcard,
                                                                    callback: function (panel, tool, event) {
                                                                        me.onItemEditCardActionClick(panel.up());
                                                                    },
                                                                    autoEl: {
                                                                        'data-testid':
                                                                            'relations-fieldset-editcardaction'
                                                                    },
                                                                    bind: {
    ...
    onAddRelationBtnClick: function (button, eOpts) {
        ...
        const domain = vm.get('domain');
        const direction = vm.get('direction') === '_1' ? 'inverse' : 'direct';

        var popup;
        const title = vm.get('basetitle');
        const config = {
    ...
    privates: {
                ...
                buttons: [
                    {
                        ...
                        autoEl: { 'data-testid': 'relations-fieldset-editcard-cancel' },
                    ...
                    },
                    {
                        ...
                        autoEl: { 'data-testid': 'relations-fieldset-editcard-save' },
                        formBind: true,
                        handler: function (btn, event) {
        ...
        onItemEditRelationCardClick: function (view) {
            ...
            this.loadRelationStore(vm.get('allRelations')).then(function (store) {
                const relationItem = store.findRecord('_destinationId', vm.get('theObject').getId());
        ...
        loadRelationStore: function (relationStore) {
            ...
            if (!relationStore.isLoaded() || Ext.Object.isEmpty(relationStore.getAdvancedFilter().getAttributes())) {
                relationStore
                    .getAdvancedFilter()
                    .addAttributeFilter('_type', 'equal', this.getViewModel().get('domain.name'));
