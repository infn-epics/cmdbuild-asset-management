(function () {
    Ext.define('CMDBuildUI.locales.cs.Locales', {
        requires: ['CMDBuildUI.locales.cs.LocalesAdministration'],
        override: 'CMDBuildUI.locales.Locales',
        singleton: true,
        localization: 'cs',
        administration: CMDBuildUI.locales.cs.LocalesAdministration.administration,
        arialabels: {
            buttonplaceholder: 'Tlačítko',
            iframeplaceholder: 'Textový editor',
            languageselector: 'Výběr jazyka',
            resizepanel: {
                horizontally: 'Změnit velikost panelů vodorovně',
                vertically: 'Změnit velikost panelů svisle'
            },
            usermenu: 'Uživatelské menu'
        },
        attachments: {
            add: 'Přidat přílohu',
            attachmenthistory: 'Historie přílohy',
            author: 'Autor',
            authordescription: 'Autor popis',
            browse: 'Prohlížet &hellip;',
            category: 'Kategorie',
            code: 'Kód',
            creationdate: 'Datum vytvoření',
            deleteattachment: 'Smazat přílohu',
            deleteattachment_confirmation: 'Opravdu chcete odstranit tuto přílohu?',
            description: 'Popis',
            download: 'Stáhnout',
            dropfiles: 'Sem přetáhněte soubory',
            editattachment: 'Upravit přílohu',
            errordms: 'DMS je vypnutý a není možné ukládat soubory',
            file: 'Soubor',
            filealreadyinlist: 'Soubor {0} se již nachází v seznamu.',
            filename: 'Název souboru',
            fileview: 'Zobrazit přílohu',
            invalidfiles: 'Odstraňte neplatné soubory',
            majorversion: 'Hlavní verze',
            maxsize: 'Maximální povolená velikost souboru je {0} MB.',
            metadata: 'Metadata',
            modificationdate: 'Datum změny',
            new: 'Nová příloha',
            nocategory: 'Nezařazené',
            preview: 'Náhled',
            removefile: 'Odstranit soubor',
            statuses: {
                empty: 'Prázdný soubor',
                error: 'Chyba',
                extensionNotAllowed: 'Přípona souboru není povolena',
                loaded: 'Načteny',
                ready: 'Připraven',
                toolarge: 'Příliš velký'
            },
            successupload: 'bylo nahraných {0} příloh.',
            uploadfile: 'Nahrát soubor ...',
            version: 'Verze',
            viewhistory: 'Zobrazit historii přílohy',
            viewmetadata: 'Zobrazit metadata přílohy',
            warningmessages: {
                atleast: '{0}: byly načteny přílohy {1} typu "{2}". Tato kategorie očekává nejméně {3} přílohy',
                exactlynumber: '{0}: byly načteny přílohy {1} typu "{2}". Tato kategorie očekává {3} přílohy',
                maxnumber: '{0}: Byla načtena příloha typu {1} typu "{2}". Tato kategorie očekává nejvíce {3} přílohy'
            },
            wrongfileextension: 'přípona souboru {0} není povolena'
        },
        bim: {
            bimViewer: 'BIM Prohlížeč',
            card: {
                label: 'Karta'
            },
            ifcproperties: {
                BOUNDING_BOX_SIZE_ALONG_X: 'Velikost ohraničeného boxu podél osy X',
                BOUNDING_BOX_SIZE_ALONG_Y: 'Velikost ohraničeného boxu podél osy Y',
                BOUNDING_BOX_SIZE_ALONG_Z: 'Velikost ohraničeného boxu podél osy Z',
                calculated: 'Kalkulované',
                description: 'Popis',
                guid: 'GUID',
                label: 'Vlastnosti',
                name: 'Název',
                type: 'Typ',
                uuid: 'UUID'
            },
            layers: {
                label: 'Vrstvy',
                menu: {
                    hideAll: 'Skrýt všechny',
                    hideAllLayers: 'Skrýt všechny vrstvy',
                    hideAllXeokit: 'Skrýt vše {0}',
                    showAll: 'Zobrazit všechny',
                    showAllLayers: 'Zobrazit všechny vrstvy',
                    showAllXeokit: 'Zobrazit vše {0}'
                },
                name: 'Název',
                quantity: 'Množství',
                visibility: 'Viditelnost'
            },
            menu: {
                camera: 'Kamera',
                disableslice: 'Zakázat řez objektu',
                enableslice: 'Povolit řez objektu',
                frontView: 'Pohled zepředu',
                mod: 'Ovládací prvky prohlížeče',
                ortho: 'Přepnout na ortogonální zobrazení',
                orthographic: 'Ortografická kamera',
                pan: 'Rolování',
                panXeokit: 'Viewer controls: scroll (Shift)',
                perspe: 'Přepnout na perspektivní zobrazení',
                perspective: 'Perspektivní kamera',
                resetSlice: 'Resetovat řez',
                resetView: 'Obnovit pohled',
                rotate: 'Otočit',
                rotateXeokit: 'Viewer controls: otočiť',
                save: 'Uložit zobrazení',
                shortcuts: {
                    down: 'Otočit dolů',
                    left: 'Otočit doleva',
                    right: 'Otočit doprava',
                    title: 'Klávesové skratky',
                    up: 'Otočit nahoru',
                    zoomin: 'Přiblížit',
                    zoomout: 'Oddálit'
                },
                sideView: 'Pohled z boku',
                threeD: 'Přepnout na 3D zobrazení',
                topView: 'Pohled shora',
                twoD: 'Přepnout na 2D zobrazení'
            },
            navCube: {
                back: 'Zpět',
                bottom: 'Spodek',
                front: 'Přední',
                left: 'Vlevo',
                right: 'Vpravo',
                top: 'Top'
            },
            showBimCard: 'Otevřít 3D prohlížeč',
            tree: {
                arrowTooltip: 'Vybírat prvek',
                label: 'Strom-Hierarchicky',
                multipleElements: 'Vybrat prvky',
                open_card: 'Otevřít související kartu',
                root: 'IFC Root'
            }
        },
        bulkactions: {
            abort: 'Zrušit vybrané položky',
            addfields: 'Přidat pole',
            alertdownloadattachments: 'Některé přílohy nebudou staženy, protože ještě nebyly uloženy na server',
            alertnoattachmentsdownload: 'Nebudou staženy žádné přílohy, protože ještě nebyly uloženy na server',
            cancelselection: 'Zrušit výběr',
            confirmabort: 'Rušíte {0} instance procesu. Jste si jistý, že chcete pokračovat?',
            confirmdelete: 'Odstraňujete {0} karet. Jste si jistý, že chcete pokračovat?',
            confirmdeleteattachements: 'Odstraňujete přílohy {0}. Jste si jistý, že chcete pokračovat?',
            confirmedit: 'Upravujete {0} pro {1} karty. Jste si jistý, že chcete pokračovat?',
            delete: 'Odstranit vybrané položky',
            download: 'Stáhnout vybrané přílohy',
            downloadall: 'Stáhnout všechny přílohy',
            edit: 'Upravit vybrané položky',
            missingfields: 'Pro správné ověření formuláře je nutné přidat následující pole: {0}',
            noattributeselected: 'Vyberte prosím alespoň jeden atribut.',
            selectall: 'Vybírat všechny položky'
        },
        calendar: {
            active_expired: 'Aktivní / prodlením',
            add: 'Přidat',
            advancenotification: 'Oznámení dní předem',
            allcategories: 'Všechny kategorie',
            alldates: 'Všechna data',
            associatedcard: 'Přidružená karta',
            calculated: 'Počítáno',
            calendar: 'Kalendář',
            cancel: 'Označit jako zrušené',
            category: 'Kategorie',
            class: 'Třída',
            cm_confirmcancel: 'Opravdu chcete označit jako zrušené vybrané plány?',
            cm_confirmcomplete: 'Opravdu chcete označit jako dokončené vybrané plány?',
            cm_markcancelled: 'Označit vybrané plány jako zrušené',
            cm_markcomplete: 'Označit vybrané plány jako dokončené',
            complete: 'Označit jako vykonáno',
            completed: 'Dokončené',
            date: 'Datum',
            days: 'Dny',
            delaybeforedeadline: 'Zpoždění před termínem',
            delaybeforedeadlinevalue: 'Zpoždění před termínem - Hodnota',
            description: 'Popis',
            editevent: 'Upravit plán',
            enddate: 'Datum ukončení',
            endtype: 'Typ ukončení',
            event: 'Plán',
            execapplyfilter: 'Nelze spustit applyAdvanceFilter, protože existuje více než jedno úložiště',
            execremovefilter: 'Nelze spustit removeAdvanceFilter, protože existuje více než jedeno úložiště',
            executiondate: 'Datum provedení',
            frequency: 'Frekvence',
            frequencymultiplier: 'Frekvence multiplikátor',
            grid: 'Tabulka',
            leftdays: 'Zbývající dny',
            londdescription: 'Úplný popis',
            manual: 'Ručně',
            maxactiveevents: 'Maximální počet aktivních událostí',
            messagebodydelete: 'Chcete odstranit pravidlo plánovače?',
            messagebodyplural: 'Existují {0} pravidla plánování',
            messagebodyrecalculate: 'Chcete přepočítat pravidlo plánů s novým datem?',
            messagebodysingular: 'Existuje {0} pravidlo plánování',
            messagetitle: 'Plán přepočet',
            missingdays: 'Chybějící dny',
            next30days: 'Dalších 30 dní',
            next7days: 'Dalších 7 dní',
            notificationtemplate: 'Šablona použitá na notifikaci',
            notificationtext: 'Text oznámení',
            occurencies: 'Počet událostí',
            operation: 'Provozní',
            partecipantgroup: 'Skupina účastníků',
            partecipantuser: 'Účastnický uživatel',
            priority: 'Priorita',
            recalculate: 'Přepočítat',
            referent: 'Referent',
            scheduler: 'Plánovač',
            sequencepaneltitle: 'Generovat plány',
            startdate: 'Datum začátku',
            status: 'Status',
            today: 'Dnes',
            type: 'Typ',
            viewevent: 'Zobrazit plán',
            waiteventgeneration: 'Generování událostí. Počkejte prosím na dokončení.',
            widgetcriterion: 'Výpočtové kritérium',
            widgetemails: 'E-Maily',
            widgetgotocard: 'Přejděte na zdrojovou kartu',
            widgetsourcecard: 'Zdrojová karta'
        },
        chat: {
            title: 'Chat'
        },
        classes: {
            cards: {
                addcard: 'Přidat kartu',
                clone: 'Klonovat',
                clonewithrelations: 'Klonovat kartu a relace',
                deletebeaware: 'Uvědomte si, že:',
                deleteblocked: 'V odstranění není možné pokračovat, protože existují relace s doménou {0}.',
                deletecard: 'Smazat kartu',
                deleteconfirmation: 'Opravdu chcete odstranit tuto kartu?',
                deleterelatedcards: 'Také bude odstraněných {0} propojených karet',
                deleterelations: 'relace s {0} kartami budou odstraněny',
                label: 'Karty',
                modifycard: 'Upravit kartu',
                opencard: 'Otevřít kartu',
                print: 'Vytisknout kartu'
            },
            simple: 'Jednoduchá',
            standard: 'Wtandardní'
        },
        common: {
            actions: {
                add: 'Přidat',
                apply: 'Použít',
                cancel: 'Zrušit',
                clear: 'Vymazat',
                close: 'Zavřít',
                copy: 'Kopírovat',
                copyerrormessage: 'Zkopírovat chybovou zprávu do schránky',
                delete: 'Smazat',
                edit: 'Upravit',
                execute: 'Provést',
                forward: 'Dál',
                help: 'Pomoc',
                load: 'Načíst',
                open: 'Otevřít',
                previous: 'Předchozí',
                refresh: 'Obnovit údaje',
                remove: 'Odstranit',
                save: 'Uložit',
                saveandapply: 'Uložit a použít',
                saveandclose: 'Uložit a zavřít',
                search: 'Vyhledávání',
                searchtext: 'Vyhledat ...'
            },
            and: 'a',
            attributes: {
                boolfalse: 'FALSE',
                booltrue: 'TRUE',
                link: {
                    errlabel: 'Označení je povinné',
                    errurl: 'Pokud označení není prázdne, je vyžadována adresa URL',
                    label: 'Označení',
                    url: 'Url'
                },
                nogroup: 'Základní údaje'
            },
            dates: {
                date: 'd/m/R',
                datetime: 'd/m/R H:m:s',
                time: 'H:m:s'
            },
            editor: {
                clearhtml: 'Clear HTML',
                expand: 'Rozbalit editor',
                reduce: 'Zmenšit editor',
                signature: 'Přidat podpis',
                unlink: 'Odpojit',
                unlinkmessage: 'Transformujte vybraný hypertextový odkaz na text.'
            },
            grid: {
                disablemultiselection: 'Zakázat výběr více položek',
                enamblemultiselection: 'Povolit výběr více položek',
                export: 'Exportovat údaje',
                filterremoved: 'Aktuální filtr byl odstraněn',
                import: 'Importovat údaje',
                itemnotfound: 'Položka nebyla nalezena',
                list: 'Seznam',
                opencontextualmenu: 'Otevřít kontextové menu',
                print: 'Tisknout',
                printcsv: 'Tisk jako CSV',
                printmap: 'Tisk mapy',
                printodt: 'Tisk jako ODT',
                printpdf: 'Tisk jako PDF',
                row: 'Položka',
                rows: 'Položky',
                subtype: 'Podtyp',
                thematism: 'Tematismus / Témata'
            },
            tabs: {
                activity: 'Činnost',
                attachment: 'Příloha',
                attachments: 'Přílohy',
                card: 'Karta',
                clonerelationmode: 'Režim klonování relací',
                details: 'Podrobnosti',
                emails: 'E-Maily',
                history: 'Historie',
                notes: 'Poznámky',
                relations: 'Relace',
                schedules: 'Plány',
                sources: 'Zdroje'
            }
        },
        dashboards: {
            tools: {
                download: 'Stáhnout graf',
                downloaddata: 'Stáhnout data grafu',
                gridhide: 'Skrýt tabulku údajů',
                gridshow: 'Zobrazit tabulku údajů',
                openinpopup: 'Otvřít v popup okně',
                parametershide: 'Skrýt parametry údajů',
                parametersshow: 'Zobrazit tabulku parametrů',
                reload: 'Opět načíst'
            }
        },
        emails: {
            abort: 'Zrušit',
            addattachmentsfromdocumentarchive: 'Přidat přílohy z archivu dokumentů',
            alredyexistfile: 'Soubor s tímto názvem již existuje',
            archivingdate: 'Datum archivování',
            attachfile: 'Přiložit soubor',
            bcc: 'Bcc',
            cc: 'Cc',
            composeemail: 'Vytvořit e-mail',
            composefromtemplate: 'Vytvořit ze šablony',
            delay: 'Zpoždění',
            delays: {
                day1: 'Za 1 den',
                days2: 'Za 2 dny',
                days4: 'Za 4 dny',
                hour1: '1 hodinu',
                hours2: '2 hodiny',
                hours4: '4 hodiny',
                month1: 'Za 1 měsíc',
                negativeday1: '1 den předtím',
                negativedays2: '2 dny předtím',
                negativedays4: '4 dny předtím',
                negativehour1: '1 hodinu předtím',
                negativehours2: '2 hodiny předtím',
                negativehours4: '4 hodiny předtím',
                negativemonth1: '1 měsíc předtím',
                negativeweek1: '1 týden předtím',
                negativeweeks2: '2 týdny předtím',
                none: 'Žádné',
                week1: 'Za 1 týden',
                weeks2: 'Za 2 týdny'
            },
            dmspaneltitle: 'Vyberte přílohy z databáze',
            edit: 'Upravit',
            from: 'Od',
            gridrefresh: 'Obnovit tabulku',
            groupbystatus: 'Seskupeno podle stavu',
            keepsynchronization: 'Ponechat synchronizaci',
            maxsize: 'Maximální celková povolená velikost pro přílohy je {0}.',
            message: 'Zpráva',
            regenerateallemails: 'Obnovit všechny E-maily',
            regenerateemail: 'Obnovit E-mail',
            reloadconfirmationmessage: 'E-maily ve stavu konceptu budou smazány. Chcete pokračovat?',
            remove: 'Odstranit',
            remove_confirmation: 'Opravdu chcete odstranit tento e-mail?',
            reply: 'Odpovědět',
            replyprefix: 'Na {0}, {1} napsal:',
            searchattachment: 'Vyhledat přílohu',
            searchcard: 'Vyhledat kartu',
            selectaclass: 'Vybírat třídu',
            sendemail: 'Odeslat E-Mail',
            sendingemail: 'Odesílání e-mailu',
            signature: 'Podpis',
            size: 'Velikost',
            status: 'Stav',
            statuses: {
                draft: 'Návrh',
                error: 'Chyba',
                new: 'Nový',
                outgoing: 'Odesílané',
                received: 'Přijaté',
                sent: 'Odeslané',
                skipped: 'Přeskočeno'
            },
            subject: 'Předmět',
            templateattachments: 'Tento e-mail obsahuje přílohy automaticky nahrané pomocí šablony',
            to: 'Komu',
            view: 'Náhled'
        },
        errors: {
            actionnotallowed: 'Akce není povolena',
            autherror: 'Chybné uživatelské jméno nebo heslo',
            classnotfound: 'Třída {0} se nenašla',
            fieldrequired: 'Toto pole je povinné',
            invalidfilter: 'Neplatný filtr',
            nodatatoexport: 'Graf nemá žádná data k exportu',
            notfound: 'Položka se nenašla'
        },
        filters: {
            actions: 'Akce',
            addfilter: 'Přidat filtr',
            any: 'Alespoň jeden',
            attachments: 'Přílohy',
            attachmentssearchtext: 'Přílohy text vyhledávání',
            attribute: 'Zvolit atribut',
            attributes: 'Atributy',
            clearfilter: 'Vynulovat filtr',
            clone: 'Klonovat',
            copyof: 'Kopie',
            currentgroup: 'Aktuální skupina',
            currentuser: 'Aktuální uživatel',
            defaultset: 'Nastavit jako výchozí',
            defaultunset: 'Zrušit výchozí nastavení',
            description: 'Popis',
            domain: 'Doména',
            errorname: 'Název filtru se již používá',
            filterdata: 'Filtrovat údaje',
            fromfilter: 'Z filtru',
            fromselection: 'Z výběru',
            group: 'Skupina',
            ignore: 'Ignorovat',
            migrate: 'Migrovat',
            name: 'Název',
            newfilter: 'Nový filtr',
            noone: 'Žádný',
            operator: 'Pravidla',
            operators: {
                and: 'A',
                beginswith: 'Začíná s',
                between: 'Mezi',
                contained: 'Obsažený',
                containedorequal: 'Obsažený nebo rovný',
                contains: 'Obsahuje',
                containsorequal: 'Obsahuje nebo rovné',
                descriptionbegin: 'Popis začíná s',
                descriptioncontains: 'Popis obsahuje',
                descriptionends: 'Popis končí s',
                descriptionnotbegin: 'Popis se nezačíná s',
                descriptionnotcontain: 'Popis neobsahuje',
                descriptionnotends: 'Popis nekončí s',
                different: 'Rozdílné',
                doesnotbeginwith: 'Nezačíná s',
                doesnotcontain: 'Neobsahuje',
                doesnotendwith: 'Nekončí s',
                endswith: 'Končí s',
                equals: 'Rovná se',
                greaterthan: 'Větší než',
                isnotnull: 'Není prázdný',
                isnull: 'Je prázdný',
                lessthan: 'Méně než',
                or: 'Nebo'
            },
            relations: 'Relace',
            removeattribute: 'Odstranit atribut',
            removeblock: 'Odstranit blok',
            type: 'Typ',
            typeinput: 'Vstupní parametr',
            typeorchoosetheattributename: 'Zadejte nebo vyberte název atributu',
            user: 'Uživatel',
            value: 'Hodnota'
        },
        gis: {
            card: 'Karta',
            cardsMenu: 'Nabídka karet',
            code: 'Kód',
            contextmenu: {
                coordinates: 'Získat souřadnice',
                coordinatescopiedmessage: 'Souřadnice zkopírovány do schránky'
            },
            description: 'Popis',
            extension: {
                errorCall: 'Chyba',
                noResults: 'Žádné výsledky'
            },
            externalServices: 'Externí služby',
            geographicalAttributes: 'Geografické atributy',
            geoserverLayers: 'Geoserver vrstvy',
            labels: {
                all: 'Všechny',
                alltitle: 'Zobrazit všechny označení',
                alltitleclass: 'Zobrazit všechny označení třídy',
                clear: 'Vymazat hledání',
                hidden: 'Skrytý',
                hiddentitle: 'Skrýt označení',
                hideinfowindow: 'Skrýt informační okno',
                label: 'Označení',
                search: 'Hledat na mapě',
                selected: 'Vybraný',
                selectedtitle: 'Zobrazit označení pro vybranou položku',
                showinfowindow: 'Zobrazit informační okno'
            },
            layers: 'Vrstvy',
            layersTab: {
                clearPreferences: 'Vymazat předvolby vrstev',
                preferencesCleared: 'Předvolby vrstev vymazány',
                preferencesSaved: 'Předvolby vrstev uloženy',
                savePreferences: 'Uložit předvolby vrstev',
                updatePreferences: 'Aktualizujte předvolby vrstev'
            },
            list: 'Seznam',
            longpresstitle: 'Geografické elementy v oblasti',
            map: 'Mapa',
            mapServices: 'Mapové služby',
            menu: {
                font: 'Velikost písma',
                options: 'Možnosti'
            },
            position: 'Místo',
            root: 'Hlavní',
            sourceMap: {
                default: 'Použít výchozí zobrazení',
                satellite: 'Použít satelitní zobrazení'
            },
            tree: 'Strom-Hierarchicky',
            type: 'Typ',
            view: 'Náhled',
            zoom: 'Zoom'
        },
        history: {
            activityname: 'Činnost název',
            activityperformer: 'Činnost vykonavatel',
            attributes: 'Atributy ({0})',
            begindate: 'Datum začátku',
            enddate: 'Datum ukončení',
            fieldname: 'Název pole',
            fulldata: 'Úplné údaje',
            onlychanges: 'Pouze změny',
            otherrelations: 'Jiné relace',
            processstatus: 'Status',
            referencerelation: 'Referenční relace',
            relation: 'Relace',
            system: 'Systém',
            type: 'Typ',
            user: 'Uživatel',
            userdescription: 'Popis uživatele',
            value: 'Hodnota',
            viewmode: 'Režim zobrazení'
        },
        iconscategories: {
            accessibility: 'Přístupnost',
            animals: 'Zvířata',
            arrows: 'Šipky',
            audiovideo: 'Audio a video',
            automotive: 'Automobilový průmysl',
            autumn: 'Podzim',
            beverage: 'Nápoje',
            buildings: 'Budovy',
            business: 'Podnikání',
            charity: 'Charita',
            chat: 'Chat',
            chess: 'Šachy',
            childhood: 'Dětství',
            code: 'Kód',
            communication: 'Komunikace',
            computers: 'Počítače',
            construction: 'Stavebnictví',
            currency: 'Měna',
            datetime: 'Datum a čas',
            design: 'Design',
            editors: 'Editory',
            education: 'Vzdělávání',
            emoji: 'Emoji',
            energy: 'Energie',
            files: 'Soubory',
            finance: 'Finance',
            food: 'Jídlo',
            fruitvegetable: 'Ovoce a zelenina',
            games: 'Hry',
            gamingtabletop: 'Deskové hry',
            gender: 'Pohlaví',
            general: 'Obecné',
            halloween: 'Halloween',
            hands: 'Ruce',
            hotel: 'Hotel',
            household: 'Domácnost',
            images: 'Obrázky',
            interfaces: 'Rozhraní',
            logistics: 'Logistika',
            maps: 'Mapy',
            maritime: 'Námořní doprava',
            marketing: 'Marketing',
            mathematics: 'Matematika',
            medical: 'Medicína',
            moving: 'Stěhování',
            music: 'Hudba',
            objects: 'Strom-Hierarchicky',
            paymentsshopping: 'Platby a nakupování',
            political: 'Politika',
            religion: 'Náboženství',
            sciencefiction: 'Sci-fi',
            security: 'Bezpečnost',
            shapes: 'Tvary',
            shopping: 'Nakupování',
            social: 'Sociální',
            spinners: 'Načítání (spinnery)',
            sports: 'Sport',
            spring: 'Jaro',
            status: 'Status',
            toggle: 'Přepínač',
            travel: 'Cestování',
            userspeople: 'Uživatelé a lidé',
            vehicles: 'Vozidla',
            weather: 'Počasí',
            winter: 'Zima'
        },
        importexport: {
            database: {
                uri: 'Database URI',
                user: 'Database Uživatel'
            },
            downloadreport: 'Převzít správu',
            emailfailure: 'Při odesílání e-mailu se vyskytla chyba!',
            emailmessage: 'Přiložený přehled importu souboru "{0}" v datu {1}',
            emailsubject: 'Import údajů zprávy',
            emailsuccess: 'E-mail byl úspěšně odeslán!',
            export: 'Export',
            exportalldata: 'Všechny údaje',
            exportfiltereddata: 'Pouze údaje odpovídající tabulkovému filtru',
            gis: {
                importconfiguration: 'Importovat konfiguraci',
                shapeimportdisabled: 'Import tvarů není pro tuto šablonu povolen',
                shapeimportenabled: 'Import tvarů je povolen pro tuto šablonu'
            },
            ifc: {
                card: 'Karta',
                project: 'Projekt',
                sourcetype: 'Import z'
            },
            import: 'Import',
            importresponse: 'Importovat odpověď',
            response: {
                created: 'Vytvořené karty',
                deleted: 'Smazané karty',
                errors: 'Chyby',
                linenumber: 'Číslo řádku',
                message: 'Zpráva',
                modified: 'Upravené karty',
                notprocessed: 'Nezpracované karty',
                processed: 'Zpracované řádky',
                recordnumber: 'Číslo záznamu',
                unchanged: 'Nezměněné karty'
            },
            sendreport: 'Odeslat zprávu',
            template: 'Šablona',
            templatedefinition: 'Definice šablony'
        },
        joinviews: {
            active: 'Aktivní',
            addview: 'Přidat pohled',
            alias: 'Alias',
            attribute: 'Atribut',
            attributecustomization: 'Atributy přizpůsobení',
            attributes: 'Atributy',
            attributeschoice: 'Atributy výběr',
            attributesof: 'Atributy: {0}',
            createview: 'Vytvořit pohled',
            datasorting: 'Setřídit údaje',
            delete: 'Odstranit',
            deleteview: 'Odstranit pohled',
            deleteviewconfirm: 'Opravdu chcete odstranit tento pohled?',
            description: 'Popis',
            disable: 'Zakázat',
            domainalias: 'Alias domény',
            domainsof: 'Domény {0}',
            edit: 'Upravit',
            editview: 'Upravit konfiguraci zobrazení',
            enable: 'Povolit',
            fieldsets: 'Skupiny polí',
            filters: 'Filtry',
            generalproperties: 'Obecné vlastnosti',
            group: 'Skupina',
            innerjoin: 'Vnitřní spojení',
            jointype: 'Typ spojení',
            joinview: 'Pohled ze spojení',
            klass: 'Třída',
            manageview: 'Spravovat pohledy',
            masterclass: 'Master třída',
            masterclassalias: 'Master třída alias',
            name: 'Název',
            newjoinview: 'Nový pohled ze spojení',
            outerjoin: 'Vnější spojení',
            pleaseseleceavalidmasterclass: 'Vyberte platnou master třídu',
            refreshafteredit: 'Chcete obnovit stránku, abyste viděli změny?',
            selectatleastoneattribute: 'Vyberte prosím alespoň jeden atribut',
            selectdisplayattributegrid:
                'Vyberte prosím alespoň jeden atribut pro zobrazení v tabulce a jeden v redukované mřížce',
            showingrid: 'Zobrazit v tabulce',
            showinreducedgrid: 'Zobrazit v zmenšené tabulce',
            targetalias: 'Alias ​​cílové třídy'
        },
        login: {
            buttons: {
                login: 'Přihlásit',
                logout: 'Změnit uživatele'
            },
            fields: {
                group: 'Skupina',
                language: 'Jazyk',
                password: 'Heslo',
                tenants: 'Tenanti',
                username: 'Uživatelské jméno'
            },
            loggedin: 'Přihlášený',
            sso: {
                loginfailure: 'Selhání přihlášení',
                loginwith: 'Přihlášení se {0}',
                or: 'nebo',
                usernotenabled: 'Uživatel není povolen'
            },
            title: 'Přihlášení',
            welcome: 'Vítej zpět {0}.'
        },
        main: {
            administrationmodule: 'Administrační modul',
            alerts: 'Upozornění',
            baseconfiguration: 'Základní konfigurace',
            cardlock: {
                lockedmessage: 'Tuto kartu nemůžete upravit, protože {0} se právě edituje.',
                someone: 'někdo'
            },
            changegroup: 'Změnit skupinu',
            changetenant: 'Změnit {0}',
            confirmchangegroup: 'Jste si jisti, že chcete změnit skupinu?',
            confirmchangetenants: 'Jste si jisti, že chcete změnit aktivních tenantov?',
            confirmdisabletenant: 'Opravdu chcete vypnout příznak "Ignorovat Tenantov"?',
            confirmenabletenant: 'Opravdu chcete zapnout příznak "Ignorovat Tenantov"?',
            description: 'Popis',
            ignoretenants: 'Ignorovat {0}',
            info: 'Info',
            logo: {
                cmdbuild: 'CMDBuild logo',
                cmdbuildready2use: 'CMDBuild READY2USE logo',
                companylogo: 'Logo společnosti',
                hdaitsm: 'HDA ITSM',
                openmaint: 'openMAINT logo'
            },
            logout: 'Odhlásit',
            managementmodule: 'Modul správy údajů',
            mobilediff: {
                acceptall: 'Přijmout všechny změny',
                acceptclass: 'Přijmout všechny změny {0}',
                acceptsingle: 'Přijmout změnu',
                attribute: 'Atribut',
                changedcards: 'Změněné karty ({0})',
                confirmedit: 'Měníte {0} karet. Jste si jisti, že chcete pokračovat?',
                createdcards: 'Vytvořené karty ({0})',
                current: 'Aktuální hodnota',
                diffempty: 'Není žádná změna k vyhodnocení',
                incoming: 'Nová hodnota',
                popuptitle: 'Aplikovat změny pro dataset {0}',
                rejectall: 'Zamítnout všechny změny',
                rejectclass: 'Zamítnout všechny změny {0}',
                rejectsingle: 'Zamítnout změnu',
                removedcards: 'Odstraněné karty ({0})'
            },
            multigroup: 'Multi skupina',
            multitenant: 'Multi {0}',
            navigation: 'Navigace',
            pagenotfound: 'Stránka nenalezena',
            password: {
                change: 'Změnit heslo',
                confirm: 'Potvrdit heslo',
                email: 'E-Mailová adresa',
                err_confirm: 'Heslo se neshoduje.',
                err_diffprevious: 'Heslo nemůže být totožné s předchozím.',
                err_diffusername: 'Heslo nemůže být totožné s uživatelským jménem.',
                err_length: 'Heslo musí mít nejméně {0} znaků.',
                err_reqdigit: 'Heslo musí obsahovat nejméně jednu číslici.',
                err_reqlowercase: 'Heslo musí obsahovat nejméně jeden malý znak.',
                err_requppercase: 'Heslo musí obsahovat nejméně jeden velký znak.',
                expired: 'Platnost vašeho hesla vypršela. Nyní to musíte změnit.',
                forgotten: 'Zapomněl jsem své heslo',
                new: 'Nové heslo',
                old: 'Staré heslo',
                recoverysuccess: 'Poslali jsme vám e-mail s pokyny pro obnovení hesla.',
                reset: 'Obnovit heslo',
                saved: 'Heslo bylo správně uloženo!'
            },
            pleasecorrecterrors: 'Opravte uvedené chyby!',
            preferences: {
                comma: 'Desetinná čárka',
                decimalserror: 'Pole desetinných míst musí být zadáno',
                decimalstousandserror: 'Oddělovač desetinných míst a tisíců musí být odlišný',
                default: 'Standardní',
                defaultvalue: 'Standardní hodnota',
                favouritemenulocation: 'Umístění nabídky Oblíbené',
                firstdayofweek: 'První den v týdnu',
                gridpreferencesclear: 'Vymazat předvolby tabulky',
                gridpreferencescleared: 'Předvolby tabulky byly vymazány!',
                gridpreferencessave: 'Uložte předvolby tabulky',
                gridpreferencessaved: 'Předvolby tabulky byly uloženy!',
                gridpreferencesupdate: 'Aktualizujte předvolby tabulky',
                groupemailsbystatus: 'Zobrazit e-maily seskupené podle stavu',
                labelcsvseparator: 'Oddělovač CSV',
                labeldateformat: 'Formát data',
                labeldecimalsseparator: 'Oddělovač desetinných míst',
                labellanguage: 'Jazyk',
                labelthousandsseparator: 'Oddělovač tisíců',
                labeltimeformat: 'Formát času',
                msoffice: 'Microsoft Office',
                no: 'Ne',
                notificationslabel: 'Chat & Notifikace',
                notificationssound: 'Přehrát zvuk pro upozornění',
                notificationssoundvolume: 'Hlasitost oznámení',
                period: 'Tečka',
                preferredfilecharset: 'Kódování CSV',
                preferredofficesuite: 'Preferovaný balík Office',
                sendcancellationperiod: 'Doba zrušení odeslání e-mailu (v sekundách)',
                showcollapsed: 'Zobrazit sbalené',
                showfovouritesafter: 'Zobrazit po navigačním menu',
                showfovouritesbefore: 'Zobrazit před navigačním menu',
                space: 'Mezera',
                thousandserror: 'Pole tisíců musí být zadáno',
                timezone: 'Časové pásmo',
                twelvehourformat: '12-hodinový formát',
                twentyfourhourformat: '24-hodinový formát',
                yes: 'Ano'
            },
            searchinallitems: 'Vyhledávat ve všech položkách',
            searchtenant: 'Vyhledat tenanta',
            selectdeselettenants: 'Vybrat/Zrušit výběr všech',
            treenavcontenttitle: '{0} z {1}',
            userpreferences: 'Uživatelské nastavení'
        },
        menu: {
            allitems: 'Všechny položky',
            classes: 'Třídy',
            custompages: 'Vlastní stránky',
            dashboards: 'Info panely',
            favourites: 'Oblíbené položky',
            favouritesadd: 'Přidat k oblíbeným položkám',
            favouritesremove: 'Odebrat z oblíbených',
            processes: 'Procesy',
            reports: 'Reporty',
            setasinitialpage: 'Nastavit jako úvodní stránku',
            views: 'Pohledy'
        },
        mobile: {
            config: {
                configure: 'Konfigurace mobilního zařízení',
                customercode: 'Kód zákazníka',
                devicename: 'Název zařízení',
                regenerate: 'Regenerovat',
                serverurl: 'Adresa URL serveru'
            }
        },
        notes: {
            edit: 'Upravit poznámky'
        },
        notifications: {
            delete: 'Odstranit notofikace',
            emptymessage: 'Nejsou zde žádná notifikace k zobrazení.',
            label: 'Oznámení',
            viewall: 'Zobrazit vše'
        },
        notifier: {
            attention: 'Upozornění',
            copied: 'Zkopírováno',
            copied_to_the_clipboard: 'Text byl zkopírován do schránky',
            error: 'Chyba',
            genericerror: 'Obecná chyba',
            genericerrormessage: 'Pro více informací si prosím prohlédněte systémové protokoly (logy).',
            genericinfo: 'Obecné info',
            genericwarning: 'Obecné upozornění',
            info: 'Info',
            more: '<em>More info</em>',
            success: 'Výsledek',
            warning: 'Výstraha'
        },
        patches: {
            apply: 'Použijte opravy',
            category: 'Kategorie',
            description: 'Popis',
            name: 'Název',
            patches: 'Opravy'
        },
        processes: {
            abortconfirmation: 'Jste si jisti, že chcete přerušit tento proces?',
            abortprocess: 'Přerušit proces',
            action: {
                advance: 'Dále',
                label: 'Provést'
            },
            activeprocesses: 'Aktivní procesy',
            allstatuses: 'Všechny',
            editactivity: 'Upravit aktivitu',
            openactivity: 'Otevřít aktivitu',
            resumeprocesses: 'Procesy, které nejsou pozastaveny, nelze obnovit',
            startworkflow: 'Start',
            workflow: 'Workflow'
        },
        relationGraph: {
            activity: 'činnost',
            allLabelsOnGraph: 'Všechny názvy v grafu',
            card: 'Karta',
            cardList: 'Seznam karet',
            cardRelations: 'Karta relací',
            choosenaviagationtree: 'Vyberte strom navigace',
            class: 'Třída',
            classList: 'Seznam tříd',
            compoundnode: 'Sloučený uzel',
            disable: 'Zakázat',
            edges: 'Okraje',
            enable: 'Aktivovat',
            labelsOnGraph: 'Tooltip na graf',
            level: 'Úroveň',
            nodes: 'Uzly',
            openRelationGraph: 'Otevřít graf relací',
            quantity: 'Množství',
            refresh: 'Obnovit',
            relation: 'Relace',
            relationGraph: 'Graf relací',
            reopengraph: 'Znovu otevřít graf z tohoto uzlu'
        },
        relations: {
            adddetail: 'Přidat podrobnosti',
            addrelations: 'Přidat relaci',
            attributes: 'Atributy',
            begindate: 'Datum začátku',
            cardnotpresentmessage:
                'Související karta již není k dispozici. Pokračování v úpravě bude vyžadovat výběr jiné karty.',
            code: 'Kód',
            deletedetail: 'Odstranit podrobnosti',
            deleterelation: 'Odstranit relaci',
            deleterelationconfirm: 'Opravdu chcete odstranit tuto relaci?',
            description: 'Popis',
            editcard: 'Upravit kartu',
            editdetail: 'Upravit podrobnosti',
            editrelation: 'Upravit relaci',
            errorexisting:
                'Relace "{0}" již existuje. Před vytvořením nového pořadu upravte existující pořad nebo jej odstraňte.',
            extendeddata: 'Rozšířené údaje',
            mditems: 'Položky',
            missingattributes: 'Chybí povinné atributy',
            opencard: 'Otevřít související kartu',
            opendetail: 'Zobrazit podrobnosti',
            relationdata: 'Relační údaje',
            type: 'Typ'
        },
        reports: {
            csv: 'CSV',
            download: 'Stáhnout',
            format: 'Formát',
            msgs: {
                creationerror: 'Při zahájení vytváření přehledu se něco pokazilo.',
                creationstarted: 'Bylo zahájeno vytváření přehledu. Až bude připraven ke stažení, budete upozorněni.'
            },
            odt: 'ODT',
            pdf: 'PDF',
            print: 'Tisk',
            reload: 'Opět načíst',
            rtf: 'RTF'
        },
        system: {
            data: {
                lookup: {
                    CalendarCategory: {
                        default: {
                            description: 'Standardní'
                        }
                    },
                    CalendarFrequency: {
                        daily: {
                            description: 'Denně'
                        },
                        monthly: {
                            description: 'Měsíčně'
                        },
                        once: {
                            description: 'Jednou'
                        },
                        weekly: {
                            description: 'Týdně'
                        },
                        yearly: {
                            description: 'Ročně'
                        }
                    },
                    CalendarPriority: {
                        default: {
                            description: 'Standardní'
                        }
                    }
                },
                role: {
                    nobody: {
                        description: 'Nikdo'
                    }
                }
            }
        },
        thematism: {
            addThematism: 'Přidat Tématický okruh',
            analysisType: 'Typ analýzy',
            attribute: 'Atribut',
            bordersIntervals: 'Nesprávná konfigurace limitů intervalů',
            calculateRules: 'Generovat pravidla stylu',
            clearThematism: 'Vymazat Tématický okruh',
            color: 'Barva',
            defineLegend: 'Definice legendy',
            defineThematism: 'Definice tematického okruhu',
            deletethematism: 'Opravdu chcete odstranit toto Tématickú mapu?',
            editThematism: 'Upravit konfiguraci pravidel',
            function: 'Funkce',
            generate: 'Generovat',
            geoAttribute: 'Geografický atribut',
            graduated: 'Absolvoval',
            highlightSelected: 'Zvýraznit vybranou položku',
            intervals: 'Intervaly',
            legend: 'Legenda',
            name: 'název',
            newThematism: 'Nový Tématický okruh',
            punctual: 'Přesné',
            quantity: 'Množství',
            segments: 'Segmenty',
            source: 'Zdroj',
            table: 'Tabulka',
            thematism: 'Tematické okruhy',
            value: 'Hodnota'
        },
        widgets: {
            attachment: {
                actions: {
                    exitwithoutsave: 'Odejít bez uložení',
                    stayinedit: 'Zpět k úpravám'
                },
                savebeforeexit: 'V přílohách jsou neuložené změny. Chcete je uložit před ukončením?',
                saveprocessmessage:
                    'Přílohy lze ukládat pouze na stávající karty. Stisknutím tlačítka Uložit bude vytvořena instance procesu s údaji zadanými ve formuláři.'
            },
            calendar: {
                error: 'Parametr {0} není nastaven',
                errorwrong: 'Parametr {0} není správně nastaven'
            },
            customform: {
                addrow: 'Přidat řádek',
                clonerow: 'Klonovat řádek',
                datanotvalid: 'Údaje nejsou platné',
                deleterow: 'Odstranit řádek',
                editrow: 'Upravit řádek',
                export: 'Export',
                import: 'Import',
                importexport: {
                    expattributes: 'Údaje na export',
                    file: 'Soubor',
                    filename: 'Název souboru',
                    format: 'Formát',
                    importmode: 'Režim importu',
                    keyattributes: 'Klíčové atributy',
                    missingkeyattr: 'Vyberte alespoň jeden klíčový atribut',
                    modeadd: 'Přidat',
                    modemerge: 'Sloučit',
                    modereplace: 'Nahradit',
                    separator: 'Oddělovač'
                },
                refresh: 'Obnovit výchozí hodnoty'
            },
            linkcards: {
                checkedonly: 'Jen kontrolované',
                editcard: 'Upravit kartu',
                opencard: 'Otevřít kartu',
                refreshselection: 'Použít výchozí výběr',
                togglefilterdisabled: 'Zakázat filtr tabulky',
                togglefilterenabled: 'Povolit filtr tabulky'
            },
            notsupportedinline: 'Vložený režim není pro tento typ widgetu podporován. Opravte konfiguraci widgetu.',
            required: 'Je povinna tato miniaplikace'
        }
    });

    function cleardata(obj) {
        for (var key in obj) {
            if (typeof obj[key] === 'string') {
                obj[key] = obj[key].replace(/^<em>(.+)<\/em>$/, '$1');
            } else if (typeof obj[key] === 'object') {
                cleardata(obj[key]);
            }
        }
    }
    cleardata(CMDBuildUI.locales.Locales);
})();
