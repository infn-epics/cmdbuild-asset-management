(function () {
    Ext.define('CMDBuildUI.locales.sr.Locales', {
        requires: ['CMDBuildUI.locales.sr.LocalesAdministration'],
        override: 'CMDBuildUI.locales.Locales',
        singleton: true,
        localization: 'sr',
        administration: CMDBuildUI.locales.sr.LocalesAdministration.administration,
        arialabels: {
            buttonplaceholder: 'Dugme',
            iframeplaceholder: 'Editor teksta',
            languageselector: 'Izbor jezika',
            resizepanel: {
                horizontally: 'Promeni širinu panela',
                vertically: 'Promeni visinu panela'
            },
            usermenu: 'Korisnički meni'
        },
        attachments: {
            add: 'Dodaj prilog',
            attachmenthistory: 'Istorija priloga',
            author: 'Autor',
            authordescription: 'Opis autora',
            browse: 'Pretraga i pomoć;',
            category: 'Kategorija',
            code: 'Kod',
            creationdate: 'Datum kreiranja',
            deleteattachment: 'Obriši prilog',
            deleteattachment_confirmation: 'Zaista želite da uklonite prilog?',
            description: 'Opis',
            download: 'Preuzmi',
            dropfiles: 'ovde spustite datoteke',
            editattachment: 'Modifikuj prilog',
            errordms: 'DSM je isključen i nije moguće snimati datoteke',
            file: 'Datoteka',
            filealreadyinlist: 'Datoteka {0} je već u listi.',
            filename: 'Naziv datoteke',
            fileview: 'Pogledaj prilog',
            invalidfiles: 'Ukloni nevalidne datoteke',
            majorversion: 'Glavna verzija',
            maxsize: 'Maksimalno dozvoljena veličina datoteke je {0} MB.',
            metadata: 'Metapodaci',
            modificationdate: 'Datum izmene',
            new: 'Novi prilog',
            nocategory: 'Nekategorisan',
            preview: 'Pregled',
            removefile: 'Ukloni datoteku',
            statuses: {
                empty: 'Prazna datoteka',
                error: 'Greška',
                extensionNotAllowed: 'Ekstenzija nije dozvoljena',
                loaded: 'Učitano',
                ready: 'Spreman',
                toolarge: 'Preveliko'
            },
            successupload: '{0} priloga je poslano.',
            uploadfile: 'Pošalji datoteku…',
            version: 'Verzija',
            viewhistory: 'Prikaži istoriju priloga',
            viewmetadata: 'Prikaži metapodatke priloga',
            warningmessages: {
                atleast: '{0}: učitano je {1} priloga tipa ”{2}”. Najmanje {3} priloga su neophodna za ovu kategoriju.',
                exactlynumber: '{0}: učitano je {1} priloga tipa ”{2}”. {3} priloga su neophodna za ovu kategoriju.',
                maxnumber:
                    '{0}: učitano je {1} priloga tipa ”{2}”. Najviše {3} priloga su dozvoljena za ovu kategoriju.'
            },
            wrongfileextension: 'ekstenzija {0} nije dozvoljena'
        },
        bim: {
            bimViewer: 'Bim prikaz',
            card: {
                label: 'Kartica'
            },
            ifcproperties: {
                BOUNDING_BOX_SIZE_ALONG_X: 'Dimenzija uokvirujuće zone duž X ose',
                BOUNDING_BOX_SIZE_ALONG_Y: 'Dimenzija uokvirujuče zone duž Y ose',
                BOUNDING_BOX_SIZE_ALONG_Z: 'Dimenzija uokvirujuće zone duž Z ose',
                calculated: 'Izračunato',
                description: 'Opis',
                guid: 'GUID',
                label: 'Osobine',
                name: 'Naziv',
                type: 'Tip',
                uuid: 'UUID'
            },
            layers: {
                label: 'Slojevi',
                menu: {
                    hideAll: 'Sakrij sve',
                    hideAllLayers: 'Sakrij sve slojeve',
                    hideAllXeokit: 'Sakrij sve {0}',
                    showAll: 'Prikaži sve',
                    showAllLayers: 'Prikaži sve slojeve',
                    showAllXeokit: 'Prikaži sve {0}'
                },
                name: 'Naziv',
                quantity: 'Kvantitet',
                visibility: 'Vidljivost'
            },
            menu: {
                camera: 'Kamera',
                disableslice: 'Onemogući isecanje objekta',
                enableslice: 'Omogući isecanje objekta',
                frontView: 'Prikaz spreda',
                mod: 'Kontrole prikaza',
                ortho: 'Prebaci se na ortogonalni prikaz',
                orthographic: 'Ortografska kamera',
                pan: 'Pomeranje',
                panXeokit: 'Kontorla prikaza: skroluj (Shift)',
                perspe: 'Prebaci se na prikaz sa perspektivom',
                perspective: 'Perspektivna kamera',
                resetSlice: 'Resetuj isečak',
                resetView: 'Resetuj prikaz',
                rotate: 'Rotiraj',
                rotateXeokit: 'Kontrola prikaza: rotiraj',
                save: 'Snimi prikaz',
                shortcuts: {
                    down: 'Rotiraj nadole',
                    left: 'Rotiraj ulevo',
                    right: 'Rotiraj udesno',
                    title: 'Prečice na tastaturi',
                    up: 'Rotiraj nagore',
                    zoomin: 'Uvećaj',
                    zoomout: 'Umanji'
                },
                sideView: 'Prikaz sa strane',
                threeD: 'Prebaci se na 3D prikaz',
                topView: 'Prikaz od gore',
                twoD: 'Prebaci se na 2D prikaz'
            },
            navCube: {
                back: 'Povratak',
                bottom: 'Dno',
                front: 'Napred',
                left: 'Levo',
                right: 'Desno',
                top: 'Vrh'
            },
            showBimCard: 'Otvori 3D prikaz',
            tree: {
                arrowTooltip: 'Izaberi element',
                label: 'Stablo',
                multipleElements: 'Izaberi elemente',
                open_card: 'Otvori pripadajuću karticu',
                root: 'Ifc koren'
            }
        },
        bulkactions: {
            abort: 'Odbaci označene stavke',
            addfields: 'Dodaj polje',
            alertdownloadattachments: 'Neki od priloga se neće moći preuzeti jer još nisu spremljeni na server.',
            alertnoattachmentsdownload: 'Nijedan prilog se neće moćii preuzeti jer još nisu spremljeni na server.',
            cancelselection: 'Poništi izbor',
            confirmabort: 'Prekidate izvršavanje {0} procesnih instanci. Da li ste sigurni da želite da nastavite?',
            confirmdelete: 'Brišete {0} kartica. Da li ste sigurni da želite da nastavite?',
            confirmdeleteattachements: 'Brišete {0} priloga. Da li ste sigurni da želite da nastavite?',
            confirmedit: 'Vršite izmenu na {0} od {1} kartica. Da li ste sigurni da želite da nastavite?',
            delete: 'Obriši označene stavke',
            download: 'Preuzmi označene priloge',
            downloadall: 'Preuzmi sve priloge',
            edit: 'Izmeni označene stavke',
            missingfields: 'Sledeća polja moraju se dodati za korektnu validaciju: {0}',
            noattributeselected: 'Molimo izaberite barem jedan atribut',
            selectall: 'Označi sve stavke'
        },
        calendar: {
            active_expired: 'Aktivno/Isteklo',
            add: 'Dodaj',
            advancenotification: 'Obaveštenje unapred',
            allcategories: 'Sve kategorije',
            alldates: 'Svi datumi',
            associatedcard: 'Pridružena kartica',
            calculated: 'Izračunato',
            calendar: 'Kalendar',
            cancel: 'Označi kao otkazano',
            category: 'Kategorija',
            class: 'Klasa',
            cm_confirmcancel: 'Da li ste sigurni da želite da označite selektovane rasporede kao otkazane?',
            cm_confirmcomplete: 'Da li ste sigurni da želite da označite selektovane rasporede kao završene?',
            cm_markcancelled: 'Označi selektovane rasporede otkazanim',
            cm_markcomplete: 'Označi selektovane rasporede završenim',
            complete: 'Završen',
            completed: 'Završeno',
            date: 'Datum',
            days: 'Dani',
            delaybeforedeadline: 'Odlaganje pre roka',
            delaybeforedeadlinevalue: 'Vrednost odlaganja pre roka',
            description: 'Opis',
            editevent: 'Izmeni događaj',
            enddate: 'Datum završetka',
            endtype: 'Završni tip',
            event: 'Zakaži događaj',
            execapplyfilter: 'Primena operacije applyAdvanceFilter nije moguća jer postoji više skladišta',
            execremovefilter: 'Primena operacije removeAdvanceFilter nije moguća jer postoji više skladišta',
            executiondate: 'Datum izvršavanja',
            frequency: 'Frekvencija',
            frequencymultiplier: 'Množač frekevencije',
            grid: 'Tabela',
            leftdays: 'Preostalo dana',
            londdescription: 'Puni opis',
            manual: 'Ručno',
            maxactiveevents: 'Maksimalan broj aktivnih događaja',
            messagebodydelete: 'Da li želite da uklonite pravilo za planiranje?',
            messagebodyplural: 'Postoji {0} pravila za planiranje',
            messagebodyrecalculate: 'Da li  želite da preračunate pravilo planiranja uzimajući u obzoir novi datum?',
            messagebodysingular: 'Postoji {0} pravila za planiranje',
            messagetitle: 'Prearačunavanje rasporeda',
            missingdays: 'Nedostajući dani',
            next30days: 'Narednih 30 dana',
            next7days: 'Narednih 7 dana',
            notificationtemplate: 'Obrazac korišćen za obaveštenja',
            notificationtext: 'Tekst obaveštenja',
            occurencies: 'Broj ponavljanja',
            operation: 'Operacija',
            partecipantgroup: 'Grupa učesnika',
            partecipantuser: 'Učesnik',
            priority: 'Prioritet',
            recalculate: 'Preračunaj',
            referent: 'Referent',
            scheduler: 'Planer',
            sequencepaneltitle: 'Generiši raspored (plan)',
            startdate: 'Datum početka',
            status: 'Status',
            today: 'Danas',
            type: 'Tip',
            viewevent: 'Prikaži događaj',
            waiteventgeneration: 'Događaji se generišu, molimo sačekajte da se proces završi',
            widgetcriterion: 'Kriterijum za kalkulaciju',
            widgetemails: 'Emailovi',
            widgetgotocard: 'Pređi na izvornu karticu',
            widgetsourcecard: 'Polazna kartica'
        },
        chat: {
            title: 'Razmena poruka'
        },
        classes: {
            cards: {
                addcard: 'Dodaj karticu',
                clone: 'Kloniraj',
                clonewithrelations: 'Kloniraj karticu i relacije',
                deletebeaware: 'Obratite pažnju na:',
                deleteblocked: 'Brisanje ne može biti nastavljeno jer postoje relacije sa {0}',
                deletecard: 'Ukloni karticu',
                deleteconfirmation: 'Zaista želite da obrišete karticu?',
                deleterelatedcards: 'biće obrisano i {0} povezanih kartica',
                deleterelations: 'relacije sa {0} kartica će biti obrisane',
                label: 'Kartice podataka',
                modifycard: 'Izmeni karticu',
                opencard: 'Otvori karticu',
                print: 'Štampaj karticu'
            },
            simple: 'Jednostavna',
            standard: 'Standardna'
        },
        common: {
            actions: {
                add: 'Dodaj',
                apply: 'Primeni',
                cancel: 'Odustani',
                clear: 'Obriši',
                close: 'Zatvori',
                copy: 'Kopiraj',
                copyerrormessage: 'Kopiraj poruku o grešci',
                delete: 'Ukloni',
                edit: 'Izmeni',
                execute: 'Izvrši',
                forward: 'Prosledi',
                help: 'Pomoć',
                load: 'Učitaj',
                open: 'Otvori',
                previous: 'Prethodni',
                refresh: 'Osveži podatke',
                remove: 'Ukloni',
                save: 'Snimi',
                saveandapply: 'Snimi i primeni',
                saveandclose: 'Snimi i zatvori',
                search: 'Pretraga',
                searchtext: 'Pretraga…'
            },
            and: 'i',
            attributes: {
                boolfalse: 'netačno',
                booltrue: 'tačno',
                link: {
                    errlabel: 'Oznaka je obavezna',
                    errurl: 'URL je obavezan kada je popunjena oznaka',
                    label: 'Oznaka',
                    url: 'URL'
                },
                nogroup: 'Osnovni podaci'
            },
            dates: {
                date: 'd/m/Y',
                datetime: 'd/m/Y H:i:s',
                time: 'H:i:s'
            },
            editor: {
                clearhtml: 'Čist HTML',
                expand: 'Proširi editor',
                reduce: 'Smanji editor',
                signature: 'Dodaj potpis',
                unlink: 'Ukloni vezu',
                unlinkmessage: 'Konvertuj hiperlink u tekst'
            },
            grid: {
                disablemultiselection: 'Onemogući višestruko selektovanje',
                enamblemultiselection: 'Omogući višestruko selektovanje',
                export: 'Izvezi podatke',
                filterremoved: 'Trenutni filter je uklonjen',
                import: 'Uvezi podatke',
                itemnotfound: 'Element nije pronađen',
                list: 'Lista',
                opencontextualmenu: 'Otvori kontekstni meni',
                print: 'Štampaj',
                printcsv: 'Štampaj kao CSV',
                printmap: 'Odštampaj mapu',
                printodt: 'Štampaj kao ODT',
                printpdf: 'Štampaj kao PDF',
                row: 'Stavka',
                rows: 'Stavke',
                subtype: 'Podtip',
                thematism: 'Tematizacija'
            },
            tabs: {
                activity: 'Aktivnost',
                attachment: 'Prilog',
                attachments: 'Prilozi',
                card: 'Kartica',
                clonerelationmode: 'Režim kloniranja relacija',
                details: 'Detalji',
                emails: 'E-mailovi',
                history: 'Istorija',
                notes: 'Napomene',
                relations: 'Relacije',
                schedules: 'Rasporedi',
                sources: 'Izvori'
            }
        },
        dashboards: {
            tools: {
                download: 'Grafikon preuzimanja',
                downloaddata: 'Preuzmi podatke sa grafikona',
                gridhide: 'Sakrij tabelu podataka',
                gridshow: 'Prikaži tabelu podataka',
                openinpopup: 'Otvori u popup prozoru',
                parametershide: 'Sakrij parametre podataka',
                parametersshow: 'Prikaži parametre podataka',
                reload: 'Ponovo učitaj'
            }
        },
        emails: {
            abort: 'Otkaži',
            addattachmentsfromdocumentarchive: 'Dodaj prilog iz arhive dokumenata',
            alredyexistfile: 'Datoteka s ovim imenom već postoji',
            archivingdate: 'Datum arhiviranja',
            attachfile: 'Priloži datoteku',
            bcc: 'Bcc',
            cc: 'Cc',
            composeemail: 'Kreiraj e-poštu',
            composefromtemplate: 'Kreiraj iz obrasca',
            delay: 'Odlaganje',
            delays: {
                day1: 'Za 1 dan',
                days2: 'Za 2 dana',
                days4: 'Za 4 dana',
                hour1: '1 sat',
                hours2: '2 sata',
                hours4: '4 sata',
                month1: 'Za 1 mesec',
                negativeday1: '1 dan pre',
                negativedays2: '2 dana pre',
                negativedays4: '4 dana pre',
                negativehour1: '1 sat pre',
                negativehours2: '2 sata pre',
                negativehours4: '4 sata pre',
                negativemonth1: '1 mesec pre',
                negativeweek1: '1 nedelju pre',
                negativeweeks2: '2 nedelje pre',
                none: 'Bez',
                week1: 'Za 1 nedelju',
                weeks2: 'Za 2 nedelje'
            },
            dmspaneltitle: 'Izaberi prilog iz baze podataka',
            edit: 'Izmeni',
            from: 'Od',
            gridrefresh: 'Osveži mrežu',
            groupbystatus: 'Grupisano po statusu',
            keepsynchronization: 'Održavaj sinhronizovano',
            maxsize: 'Ukupna dozvoljena veličina za priloge je {0}',
            message: 'Poruka',
            regenerateallemails: 'Generiši sve emailove ponovo',
            regenerateemail: 'Iznova generiši e-poštu',
            reloadconfirmationmessage: 'Email poruka u statusu nacrta će biti obrisana. Da li želite da nastavite?',
            remove: 'Ukloni',
            remove_confirmation: 'Zaista želite da uklonite ovaj e-mail?',
            reply: 'Odgovori',
            replyprefix: '{0}, {1} je napisao',
            searchattachment: 'Pretraži prilog',
            searchcard: 'Pretraži karticu',
            selectaclass: 'Izaberi klasu',
            sendemail: 'Pošalji e-poštu',
            sendingemail: 'E-pošta se šalje.',
            signature: 'Potpis',
            size: 'Veličina',
            status: 'Status',
            statuses: {
                draft: 'Započete',
                error: 'Greška',
                new: 'Novo',
                outgoing: 'Za slanje',
                received: 'Primljene',
                sent: 'Poslane',
                skipped: 'Preskočen'
            },
            subject: 'Subjekat',
            templateattachments: 'Ova email poruka sadrži prilog koji je obrazac automatski poslao',
            to: 'Za',
            view: 'Prikaz'
        },
        errors: {
            actionnotallowed: 'Akcija nije dozvoljena',
            autherror: 'Pogrešno korisničko ime i/ili lozinka',
            classnotfound: 'Klasa {0} ne postoji',
            fieldrequired: 'Ovo polje je obavezno',
            invalidfilter: 'Nevalidan filter',
            nodatatoexport: 'Grafikon nema podatke za eksportovanje',
            notfound: 'Element nije pronađen'
        },
        filters: {
            actions: 'Akcije',
            addfilter: 'Dodaj filter',
            any: 'Barem jedan',
            attachments: 'Prilozi',
            attachmentssearchtext: 'Tekst za pretragu priloga',
            attribute: 'Izaberi atribut',
            attributes: 'Atributi',
            clearfilter: 'Očisti filter',
            clone: 'Kloniraj',
            copyof: 'Kopija',
            currentgroup: 'Trenutna grupa',
            currentuser: 'Trenutni korisnik',
            defaultset: 'Postavi kao podrazumevanu',
            defaultunset: 'Ukloni kao podrazumevanuu',
            description: 'Opis',
            domain: 'Relacija',
            errorname: 'Postoji filter sa tim nazivom',
            filterdata: 'Filtriraj podatke',
            fromfilter: 'Iz filtera',
            fromselection: 'Iz selekcije',
            group: 'Grupa',
            ignore: 'Ignoriši',
            migrate: 'Premešta',
            name: 'Naziv',
            newfilter: 'Novi filter',
            noone: 'Nijedan',
            operator: 'Operator',
            operators: {
                and: 'I',
                beginswith: 'Koji počinju sa',
                between: 'Između',
                contained: 'Sadržan',
                containedorequal: 'Sadržan ili jednak',
                contains: 'Sadrži',
                containsorequal: 'Sadrži ili je jednak',
                descriptionbegin: 'Opis počinje sa',
                descriptioncontains: 'Opis sadrži',
                descriptionends: 'Opis se završava sa',
                descriptionnotbegin: 'Opis ne počinje sa',
                descriptionnotcontain: 'Opis ne sadrži',
                descriptionnotends: 'Opis se ne završava sa',
                different: 'Različite od',
                doesnotbeginwith: 'Koji ne počinju sa',
                doesnotcontain: 'Koji ne sadrže',
                doesnotendwith: 'Ne završava sa',
                endswith: 'Završava sa',
                equals: 'Jednake',
                greaterthan: 'Veće',
                isnotnull: 'Ne može biti null',
                isnull: 'Sa null vrednošću',
                lessthan: 'Manje',
                or: 'Ili'
            },
            relations: 'Relacije',
            removeattribute: 'Ukloni atribut',
            removeblock: 'Ukloni blok',
            type: 'Tip',
            typeinput: 'Ulazni parametar',
            typeorchoosetheattributename: 'Unesite ili odaberite naziv atributa',
            user: 'Korisnik',
            value: 'Vrednosti'
        },
        gis: {
            card: 'Kartica',
            cardsMenu: 'Meni kartica',
            code: 'Kod',
            contextmenu: {
                coordinates: 'Očitaj koordinate',
                coordinatescopiedmessage: 'Koordinate su kopirane'
            },
            description: 'Opis',
            extension: {
                errorCall: 'Greška',
                noResults: 'Nema rezultata'
            },
            externalServices: 'Spoljni servisi',
            geographicalAttributes: 'Geografski atributi',
            geoserverLayers: 'Geoserver slojevi',
            labels: {
                all: 'Svе',
                alltitle: 'Prikaži sve oznake',
                alltitleclass: 'Prikaži sve oznake klase',
                clear: 'Očisti pretragu',
                hidden: 'Skriveno',
                hiddentitle: 'Sakrij oznake',
                hideinfowindow: 'Sakrij prozor sa informacijama',
                label: 'Oznake',
                search: 'Traži na mapi',
                selected: 'Izabrano',
                selectedtitle: 'Prikaži oznake za selektovanu stavku',
                showinfowindow: 'Prikaži prozor sa informacijama'
            },
            layers: 'Slojevi',
            layersTab: {
                clearPreferences: 'Obriši podešavanja za sloj',
                preferencesCleared: 'Podešavanja sloja su obrisana',
                preferencesSaved: 'Podešavanja sloja snimljena',
                savePreferences: 'Snimi podešavanja za sloj',
                updatePreferences: 'Osveži podešavanja slojeva'
            },
            list: 'Lista',
            longpresstitle: 'Geoelementi u zoni',
            map: 'Mapa',
            mapServices: 'Servisi mapa',
            menu: {
                font: 'Veličina fonta',
                options: 'Opcije'
            },
            position: 'Pozicija',
            root: 'Koren',
            sourceMap: {
                default: 'Koristi podrazumevani prikaz',
                satellite: 'Koristi satelitski prikaz'
            },
            tree: 'Stablo navigacije',
            type: 'Tip',
            view: 'Prikaz',
            zoom: 'Uvećanje'
        },
        history: {
            activityname: 'Naziv aktivnosti',
            activityperformer: 'Izvršilac aktivnosti',
            attributes: 'Atributi ({0})',
            begindate: 'Datum početka',
            enddate: 'Datum završetka',
            fieldname: 'Naziv polja',
            fulldata: 'Svi podaci',
            onlychanges: 'Samo izmenjeno',
            otherrelations: 'Druge relacije',
            processstatus: 'Status',
            referencerelation: 'Referentne relacije',
            relation: 'Relacija',
            system: 'Sistem',
            type: 'Tip',
            user: 'Korisnik',
            userdescription: 'Opis korisnika',
            value: 'Vrednost',
            viewmode: 'Način prikaza'
        },
        iconscategories: {
            accessibility: 'Dostupnost',
            animals: 'Životinje',
            arrows: 'Strelice',
            audiovideo: 'Audio video',
            automotive: 'Automobilizam',
            autumn: 'Jesen',
            beverage: 'Piće',
            buildings: 'Zgrade',
            business: 'Poslovanje',
            charity: 'Dobročinstvo',
            chat: 'Razmena poruka',
            chess: 'Šah',
            childhood: 'Detinjstvo',
            code: 'Kod',
            communication: 'Komunikacije',
            computers: 'Računari',
            construction: 'Konstrukcije',
            currency: 'Valuta',
            datetime: 'Datum i vreme',
            design: 'Dizajn',
            editors: 'Urednici',
            education: 'Obrazovanje',
            emoji: 'Emoji',
            energy: 'Energija',
            files: 'Datoteke',
            finance: 'Finansije',
            food: 'Hrana',
            fruitvegetable: 'Voće i povrće',
            games: 'Igre',
            gamingtabletop: 'Tabla za igre',
            gender: 'Pol',
            general: 'Opšte',
            halloween: 'Noć veštica',
            hands: 'Ruke',
            hotel: 'Hotel',
            household: 'Domaćinstvo',
            images: 'Slike',
            interfaces: 'Interfejsi',
            logistics: 'Logistika',
            maps: 'Mape',
            maritime: 'Pomorstvo',
            marketing: 'Marketing',
            mathematics: 'Matematika',
            medical: 'Medicinski',
            moving: 'Kretanje',
            music: 'Muzika',
            objects: 'Stablo',
            paymentsshopping: 'Plaćanje kupovina',
            political: 'Politički',
            religion: 'Religija',
            sciencefiction: 'Naučna fantastika',
            security: 'Bezbednost',
            shapes: 'Oblici',
            shopping: 'Kupovina',
            social: 'Društveni',
            spinners: 'Rotirajuć',
            sports: 'Sportovi',
            spring: 'Proleće',
            status: 'Status',
            toggle: 'Uključi/isključi',
            travel: 'Putovanje',
            userspeople: 'Korisnici, ljudi',
            vehicles: 'Vozila',
            weather: 'Vremenska zona',
            winter: 'Zima'
        },
        importexport: {
            database: {
                uri: 'URI baze podataka',
                user: 'Korisničko ime za bazu podataka'
            },
            downloadreport: 'Preuzmi izveštaj',
            emailfailure: 'Greška prilikom slanja e-maila',
            emailmessage: 'Izveštaj o importovanju datotekte {0} priložen uz datum {1}',
            emailsubject: 'Uvezi izveštaj s podacima',
            emailsuccess: 'E-maili je uspešno poslan',
            export: 'Izvezi',
            exportalldata: 'Sve podatke',
            exportfiltereddata: 'Samo podatke koji odgovaraju filteru tabele',
            gis: {
                importconfiguration: 'Uvezi konfiguraciju',
                shapeimportdisabled: 'Uvoz oblika nije omogućen za ovaj obrazac',
                shapeimportenabled: 'Za ovaj obrazac je omogućen uvoz oblika'
            },
            ifc: {
                card: 'Kartica',
                project: 'Projekat',
                sourcetype: 'Uvoz iz'
            },
            import: 'Uvezi',
            importresponse: 'Uvezi odgovor',
            response: {
                created: 'Kreirane kartice',
                deleted: 'Obrisane kartice',
                errors: 'Greške',
                linenumber: 'Broj linije',
                message: 'Poruka',
                modified: 'Izmenjene kartice',
                notprocessed: 'Neprocesirane kartice',
                processed: 'Obrađeni redovi',
                recordnumber: 'Broj zapisa',
                unchanged: 'Nepromenjene kartice'
            },
            sendreport: 'Pošalji izveštaj',
            template: 'Šablon',
            templatedefinition: 'Definicija šablona'
        },
        joinviews: {
            active: 'Aktivan',
            addview: 'Dodaj pogled',
            alias: 'Alijas',
            attribute: 'Atribut',
            attributecustomization: 'Prilagođavanje atributa',
            attributes: 'Atributi',
            attributeschoice: 'Izbor atributa',
            attributesof: 'Atributi od {0}',
            createview: 'Kreiraj pogled',
            datasorting: 'Sortiranja podataka',
            delete: 'Obriši',
            deleteview: 'Obriši pogled',
            deleteviewconfirm: 'Da li ste sigurni da želite da uklonite ovaj pogled',
            description: 'Opis',
            disable: 'Onemogući',
            domainalias: 'Alijas relacije',
            domainsof: 'Relacije od {0}',
            edit: 'Izmeni',
            editview: 'Izmeni konfiguraciju pogleda',
            enable: 'Omogući',
            fieldsets: 'Skupovi polja',
            filters: 'Filteri',
            generalproperties: 'Opšte osobine',
            group: 'Grupa',
            innerjoin: 'Unutrašnje spajanje (INNER JOIN)',
            jointype: 'Tip spajanja (JOIN)',
            joinview: 'Pogled kreiran spajanjem (join)',
            klass: 'Klasa',
            manageview: 'Upravljanje pogledom',
            masterclass: 'Osnovna klasa',
            masterclassalias: 'Alijas osnovne klase',
            name: 'Naziv',
            newjoinview: 'Novi pogled na osnovu spajanja (join)',
            outerjoin: 'Spoljašnje spajanje (OUTER JOIN)',
            pleaseseleceavalidmasterclass: 'Izaberite validnu osnovnu klasu',
            refreshafteredit: 'Da li želite da osvežite stranu kako bi videli promene?',
            selectatleastoneattribute: 'Molimo - izaberite bar jedan atribut',
            selectdisplayattributegrid:
                'Molimo - izaberite bar jedan atribut koji će biti prikazan u tabeli i jedan koji će biti prikazivan u redukovanoj tabeli',
            showingrid: 'Prikaži u tabeli',
            showinreducedgrid: 'Prikaži u redukovanoj tabeli',
            targetalias: 'Alijas odredišne klase'
        },
        login: {
            buttons: {
                login: 'Prijavi se',
                logout: 'Promeni korisnika'
            },
            fields: {
                group: 'Grupa',
                language: 'Jezik',
                password: 'Lozinka',
                tenants: 'Klijenti',
                username: 'Korisničko ime'
            },
            loggedin: 'Prijavljen',
            sso: {
                loginfailure: 'Greška prilikom prijavljivanja',
                loginwith: 'Prijava sa {0}',
                or: 'ili',
                usernotenabled: 'Korisnik nije aktiviran'
            },
            title: 'Prijavi se',
            welcome: 'Dobro došli nazad, {0}'
        },
        main: {
            administrationmodule: 'Administracioni modul',
            alerts: 'Upozorenja',
            baseconfiguration: 'Osnovna konfiguracija',
            cardlock: {
                lockedmessage: 'Ne možete menjati ovu karticu jer je trenutno menja {0}',
                someone: 'neko'
            },
            changegroup: 'Promeni grupu',
            changetenant: 'Promeni {0}',
            confirmchangegroup: 'Zaista želite da promenite grupu?',
            confirmchangetenants: 'Zaista želite da promenite aktivnog klijenta?',
            confirmdisabletenant: 'Zaista želite da isključite oznaku „Ignoriši klijente“?',
            confirmenabletenant: 'Zaista želite da uključite oznaku „Ignoriši klijente“?',
            description: 'Opis',
            ignoretenants: 'Ignoriši {0}',
            info: 'Informacija',
            logo: {
                cmdbuild: 'CMDBuild logotip',
                cmdbuildready2use: 'CMDBuild READY2USE logotip',
                companylogo: 'Kompanijski logotip',
                hdaitsm: 'HDA ITSM',
                openmaint: 'openMAINT logotip'
            },
            logout: 'Izađi',
            managementmodule: 'Modul za upravljanje podacima',
            mobilediff: {
                acceptall: 'Prihvati sve izmene',
                acceptclass: 'Prihvati svih {0} izmena',
                acceptsingle: 'Prihvati izmenu',
                attribute: 'Atribut',
                changedcards: 'Izmenjene kartice ({0})',
                confirmedit: 'Vršite izmenu na {0} kartica. Da li ste sigurni da želite da nastavite?',
                createdcards: 'Kreirane kartice ({0})',
                current: 'Trenutna vrednost',
                diffempty: 'Nema izmena za evaluaciju',
                incoming: 'Nova vrednost',
                popuptitle: 'Primeni izmene na skupu podataka {0}',
                rejectall: 'Odbaci sve izmene',
                rejectclass: 'Odbaci svih {0} izmena',
                rejectsingle: 'Odbaci izmenu',
                removedcards: 'Uklonjenih kartica ({0})'
            },
            multigroup: 'Više grupa',
            multitenant: 'Više {0}',
            navigation: 'Navigacija',
            pagenotfound: 'Stranica nije pronađena',
            password: {
                change: 'Promeni lozinku',
                confirm: 'Potvrdi lozinku',
                email: 'E-mail dares',
                err_confirm: 'Lozinke se ne poklapaju.',
                err_diffprevious: 'Lozinka ne može biti ista kao prethodna.',
                err_diffusername: 'Lozinka ne može biti ista kao i korisničko ime.',
                err_length: 'Lozinka mora biti duga minimalno {0} karaktera.',
                err_reqdigit: 'Lozinka mora sadržati barem jednu cifru.',
                err_reqlowercase: 'Lozinka mora sadržati baerm jedno malo slovo.',
                err_requppercase: 'Lozinka mora sadržati barem jedno veliko slovo.',
                expired: 'Vaša lozinka je istekla, morate je promeniti.',
                forgotten: 'Zaboravljena lozinka',
                new: 'Nova lozinka',
                old: 'Stara lozinka',
                recoverysuccess: 'Poslali smo vam email sa instrukcijama kako da povratite lozinku.',
                reset: 'Resetuj lozinku',
                saved: 'Loznika je uspešno snimljena!'
            },
            pleasecorrecterrors: 'Molimo korigujte navedene greške!',
            preferences: {
                comma: 'Zapeta',
                decimalserror: 'Decimalni deo mora postojati',
                decimalstousandserror: 'Decimalni separator i separator hiljada ne smeju biti isti',
                default: 'Podrazumevani',
                defaultvalue: 'Podrazumevana vrednost',
                favouritemenulocation: 'Lokacija menija omiljenih stavki',
                firstdayofweek: 'Prvi dan u sedmici',
                gridpreferencesclear: 'Poništi podešavanja tabele',
                gridpreferencescleared: 'Podešavanje tabele poništeno!',
                gridpreferencessave: 'Snimi podešavanje tabele',
                gridpreferencessaved: 'Podešavanje tabele snimljeno!',
                gridpreferencesupdate: 'Osveži podešavanje tabele',
                groupemailsbystatus: 'Prikaži e-poštu grupisanu po statusu',
                labelcsvseparator: 'CSV separator',
                labeldateformat: 'Format datuma',
                labeldecimalsseparator: 'Decimalni separator',
                labellanguage: 'Jezik',
                labelthousandsseparator: 'Separator hiljada',
                labeltimeformat: 'Format vremena',
                msoffice: 'Microsoft Office',
                no: 'Ne',
                notificationslabel: 'Poruke i obaveštenja',
                notificationssound: 'Zvučna notifikacija kada stigne obaveštenje',
                notificationssoundvolume: 'Jačina zvuka notifikaciija',
                period: 'Tačka',
                preferredfilecharset: 'kodna strana CSV datoteke',
                preferredofficesuite: 'Preferirani paket kancelarijskih aplikacija',
                sendcancellationperiod: 'Period u kome je moguće otkazati slanje emaila (sekunde)',
                showcollapsed: 'Prikaži kolapsirano',
                showfovouritesafter: 'Prikaži iza navigacionog menija',
                showfovouritesbefore: 'Prikaži ispred navigacionog menija',
                space: 'Razmak',
                thousandserror: 'Hiljade moraju biti prisutne',
                timezone: 'Vremenska zona',
                twelvehourformat: '12-časovni format',
                twentyfourhourformat: '24-časovni format',
                yes: 'Da'
            },
            searchinallitems: 'Pretraga kroz sve stavke',
            searchtenant: 'Pretraga klijenata',
            selectdeselettenants: 'Selektuj/deselektuj  sve',
            treenavcontenttitle: '{0} od {1}',
            userpreferences: 'Podešavanja'
        },
        menu: {
            allitems: 'Sve stavke',
            classes: 'Klase',
            custompages: 'Posebne stranice',
            dashboards: 'Kontrolne table',
            favourites: 'Omiljene stavke',
            favouritesadd: 'Dodaj u omiljene stavke',
            favouritesremove: 'Ukloni iz liste omiljenih',
            processes: 'Kartice procesa',
            reports: 'izveštaji',
            setasinitialpage: 'Postavi kao početnu stranicu',
            views: 'Prikazi'
        },
        mobile: {
            config: {
                configure: 'Podesi mobilni uređaj',
                customercode: 'Korisnički kod',
                devicename: 'Naziv uređaja',
                regenerate: 'Iznova generiši',
                serverurl: 'URL adresa servera'
            }
        },
        notes: {
            edit: 'Izmeni napomenu'
        },
        notifications: {
            delete: 'Obriši obaveštenje',
            emptymessage: 'Nema obaveštenja za prikaz.',
            label: 'Obaveštenja',
            viewall: 'Prikaži sve'
        },
        notifier: {
            attention: 'Pažnja',
            copied: 'Kopirano',
            copied_to_the_clipboard: 'Tekst je kopiran',
            error: 'Greška',
            genericerror: 'Generička greška',
            genericerrormessage: 'Za više informacija, pregledajte sistemske logove',
            genericinfo: 'Generička informacija',
            genericwarning: 'Generičko upozorenje',
            info: 'Informacija',
            more: '<em>More info</em>',
            success: 'Uspeh',
            warning: 'Pažnja'
        },
        patches: {
            apply: 'Primeni ispravke',
            category: 'Kategorija',
            description: 'Opis',
            name: 'Naziv',
            patches: 'Ispravke'
        },
        processes: {
            abortconfirmation: 'Da li ste sigurni da želite prekinuti proces?',
            abortprocess: 'Prekini proces',
            action: {
                advance: 'Dalje',
                label: 'Akcija'
            },
            activeprocesses: 'Aktivni procesi',
            allstatuses: 'Sve',
            editactivity: 'Izmeni aktivnost',
            openactivity: 'Otvori aktivnost',
            resumeprocesses: 'Procesi koji nisu susendovani ne mogu biti nastavljeni',
            startworkflow: 'Start',
            workflow: 'Radni procesi'
        },
        relationGraph: {
            activity: 'aktivnost',
            allLabelsOnGraph: 'sve oznake na grafikonu',
            card: 'Kartica',
            cardList: 'Lista kartica',
            cardRelations: 'Veza',
            choosenaviagationtree: 'Izaberi stablo navigacije',
            class: 'Klasa',
            classList: 'Lista klasa',
            compoundnode: 'Složeni čvor',
            disable: 'Isključi',
            edges: 'Grane',
            enable: 'Uključi',
            labelsOnGraph: 'opis na grafikonu',
            level: 'Nivo',
            nodes: 'Čvorovi',
            openRelationGraph: 'Otvori graf relacija',
            quantity: 'Kvantitet',
            refresh: 'Osveži',
            relation: 'Veza',
            relationGraph: 'Graf relacija',
            reopengraph: 'Ponovo otvori graf od ovog čvora'
        },
        relations: {
            adddetail: 'Dodaj detalje',
            addrelations: 'Dodaj relaciju',
            attributes: 'Atributi',
            begindate: 'Početni datum',
            cardnotpresentmessage:
                'Povezana kartica nije više dostupna. Da bi nastavili sa izmenama neophodno je da izaberete drugu karticu.',
            code: 'Kod',
            deletedetail: 'Ukloni detalj',
            deleterelation: 'Ukloni relaciju',
            deleterelationconfirm: 'Da li ste sigurni da želite da obrišete ovu relaciju?',
            description: 'Opis',
            editcard: 'Izmeni karticu',
            editdetail: 'Izmeni detalje',
            editrelation: 'Izmeni relaciju',
            errorexisting:
                'Veza "{0}" već postoji. Uradite izmene postojeće veze, ili je obrišite pre kreiranja nove veze.',
            extendeddata: 'Prošireni podaci',
            mditems: 'stavke',
            missingattributes: 'Nedostaju obavezni atributi',
            opencard: 'Otvori pripadajuću karticu',
            opendetail: 'Prikaži detalje',
            relationdata: 'Podaci relacije',
            type: 'Tip'
        },
        reports: {
            csv: 'CSV',
            download: 'Preuzmi',
            format: 'Formatiraj',
            msgs: {
                creationerror: 'Došlo je do greške prilikom pokretanja izrade izveštaja.',
                creationstarted: 'Izrada izveštaja je pokrenuta. Bićete obavešteni kada bude spremna za preuzimanje'
            },
            odt: 'ODT',
            pdf: 'PDF',
            print: 'Štampaj',
            reload: 'Ponovo učitaj',
            rtf: 'RTF'
        },
        system: {
            data: {
                lookup: {
                    CalendarCategory: {
                        default: {
                            description: 'Podrazumevano'
                        }
                    },
                    CalendarFrequency: {
                        daily: {
                            description: 'Dnevno'
                        },
                        monthly: {
                            description: 'Mesečno'
                        },
                        once: {
                            description: 'Jednom'
                        },
                        weekly: {
                            description: 'Sedmično'
                        },
                        yearly: {
                            description: 'Godišnje'
                        }
                    },
                    CalendarPriority: {
                        default: {
                            description: 'Podrazumevano'
                        }
                    }
                },
                role: {
                    nobody: {
                        description: 'Niko'
                    }
                }
            }
        },
        thematism: {
            addThematism: 'Dodaj tematizaciju',
            analysisType: 'Tip analize',
            attribute: 'Atribut',
            bordersIntervals: 'Pogrešna konfiguracija za ograničenja na periode',
            calculateRules: 'Generiši pravila stila',
            clearThematism: 'Ukloni tematizaciju',
            color: 'Boja',
            defineLegend: 'Definicija legende',
            defineThematism: 'Definicija tematizacije',
            deletethematism: 'Da li ste sigurni da želite da obrišete ovo podešavanje teme?',
            editThematism: 'Izmeni konfiguraciju pravila',
            function: 'Funkcija',
            generate: 'Generiši',
            geoAttribute: 'Geografski atributi',
            graduated: 'Diplomirao',
            highlightSelected: 'Označi selektovanu stavku',
            intervals: 'Intervali',
            legend: 'legenda',
            name: 'naziv',
            newThematism: 'Nova tematizacija',
            punctual: 'Tačan',
            quantity: 'Broj (kvantitet)',
            segments: 'Segmenti',
            source: 'Izvor',
            table: 'Tabela',
            thematism: 'Tematike',
            value: 'Vrednost'
        },
        widgets: {
            attachment: {
                actions: {
                    exitwithoutsave: 'Napusti bez snimanja',
                    stayinedit: 'Povratak na izmene'
                },
                savebeforeexit: 'Postoje nesnimljene izmene priloga. Da li želite da ih snimite pre izlaska?',
                saveprocessmessage:
                    'Prilozi mogu biti snimljeni samo na postojeće kartice. Ukoliko potvrdite snimanje, kreiraće se instanca procesa sa podacima iz forme.'
            },
            calendar: {
                error: 'Parametar {0} nije popunjen',
                errorwrong: 'Parametar {0} nije pravilno popunjen'
            },
            customform: {
                addrow: 'Dodaj red',
                clonerow: 'Kloniraj red',
                datanotvalid: 'Nevalidni podaci',
                deleterow: 'Obriši red',
                editrow: 'Izmeni red',
                export: 'Izvezi',
                import: 'Uvezi',
                importexport: {
                    expattributes: 'Podaci za izvoz',
                    file: 'Datotteka',
                    filename: 'Naziv datoteke',
                    format: 'Format',
                    importmode: 'Način uvoza',
                    keyattributes: 'Ključevi',
                    missingkeyattr: 'Molimo - izaberite bar jedan atribut-ključ',
                    modeadd: 'Dodaj',
                    modemerge: 'Spoji',
                    modereplace: 'Zameni',
                    separator: 'Separator'
                },
                refresh: 'Vrati na podrazumevane vrednosti'
            },
            linkcards: {
                checkedonly: 'Samo označene',
                editcard: 'Izmeni karticu',
                opencard: 'Otvori karticu',
                refreshselection: 'Primeni podrazumevani izbor',
                togglefilterdisabled: 'Isključi filtriranje tabele',
                togglefilterenabled: 'Uključi filtriranje tabele'
            },
            notsupportedinline: 'Inline režim nije moguć za ovaj tip komponente. Podesite konfiguraciju komponente.',
            required: 'Ova komponenta je obavezna'
        }
    });

    function cleardata(obj) {
        for (var key in obj) {
            if (typeof obj[key] === 'string') {
                obj[key] = obj[key].replace(/^<em>(.+)<\/em>$/, '$1');
            } else if (typeof obj[key] === 'object') {
                cleardata(obj[key]);
            }
        }
    }
    cleardata(CMDBuildUI.locales.Locales);
})();
